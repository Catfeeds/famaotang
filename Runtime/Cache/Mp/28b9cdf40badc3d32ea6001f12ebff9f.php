...
<?php if(($qypurview["18000"]) == "18000"): ?><div class="menu_parent">
	...
	<?php if(($qypurview["18002"]) == "18002"): ?>-->
	    <li><A href="<?php echo U('Mp/Capital/yufukuan');?>" class="<?php echo ($curr =='yufukuan' ?'curr':''); ?>" >预付款明细</A></li>
...
<div class="nav_l"><a href="<?php echo U('Mp/Index/index');?>">首页</a>　&gt;　<A href="<?php echo U('Mp/Subuser/index');?>">子用户管理</A>　&gt;　<A href="#"><?php echo ($mtitle); ?></A></div>
<div class="nav_r"></div>
</div>
<div class="height20"></div>
<div class="content">
<form action="<?php echo U('Mp/Subuser/subuseradd_save');?>"   method="post" name="fmmm" >
<input type="hidden" name="su_id" value="<?php echo ($subuserinfo["su_id"]); ?>" />
<table width="100%" border="0" cellspacing="0" cellpadding="0" >
             <tr>     
			   ...
			   <tr>     
                  <td height="40"  class="txtright" >子用户名：</td>
				  <td  class="txtleft" >
				  <?php if($subuserinfo["su_id"] == '0' ): echo ($qyuser); ?>:<input    type="text" size="25" maxlength="32"  name="su_username"  class="input"  value=""   >  * 
				  <?php else: ?>
				  <?php echo ($qyuser); ?>:<?php echo ($subuserinfo["su_username"]); endif; ?>
		      </td>
			  </tr>
			  <tr>     
                  <td height="40"  class="txtright" >密　码：</td>
				  <td  class="txtleft" >
				  <input   size="25" maxlength="32"  name="su_pwd"  class="input"  value=""  type="password"  >  * 
		      </td>
			  </tr>
			  <tr>     
                  <td height="40"  class="txtright" >名称备注：</td>
				  <td  class="txtleft" >
				  <input    type="text" size="25" maxlength="32"  name="su_name"  class="input"  value="<?php echo ($subuserinfo["su_name"]); ?>"   >  * 
		      </td>
			  </tr>
			  <?php if(($qypurview["17002"]) == "17002"): ?><tr>     
                  <td height="40"  class="txtright" >权限：</td>
				  <td  class="txtleft" >
				  
<table cellspacing="1" cellpadding="1"  class="purview" >
 <tbody>
 <?php if(is_array($purview_arr)): foreach($purview_arr as $key=>$item): ?><tr>
  <td width="15%"    class="tdl" >
    <input type="checkbox"  onclick="check('<?php echo ($item["ids"]); ?>',this);" value="<?php echo ($item["value"]); ?>" name="subadmin_purview[]"  id="parent_<?php echo ($item["value"]); ?>"   <?php echo ($item["checked"]); ?> > <?php echo ($item["pname"]); ?></td>
    <td class="tdr" >
	<?php if(is_array($item['sub'])): foreach($item['sub'] as $key2=>$item2): ?><div class="son"><input type="checkbox" class="sonbox"  value="<?php echo ($item2["value"]); ?>" name="subadmin_purview[]" onClick="checkrelevance('#parent_<?php echo ($item2["value"]); ?>',this)" <?php echo ($item2["checked"]); ?> > <?php echo ($item2["pname"]); ?></div><?php endforeach; endif; ?>
	</td>
			  ...
			  <tr>  <td height="60" ></td>
                    <td  class="txtleft"  >
 <?php if(is_array($hide_arr)): foreach($hide_arr as $key=>$item): ?><input type="hidden"  value="<?php echo ($item["value"]); ?>"  name="subhide_purview[]"  /><?php endforeach; endif; ?>
					
             <input type="submit" name="Submit" value="<?php echo ($mtitle); ?>" class="botton"   id="submit" >　　　　　<input name="" type="reset" value="返 回"  onClick="javascript:window.history.go(-1);" class="botton" >
            </td>
			  </tr>
			</table>
</form>
