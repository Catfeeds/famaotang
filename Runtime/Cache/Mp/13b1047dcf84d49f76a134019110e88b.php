...
<link rel="stylesheet" type="text/css" href="/famaotang/Public/mp/js/themes/default/easyui.css">
<link rel="stylesheet" type="text/css" href="/famaotang/Public/mp/js/themes/icon.css">
<script type="text/javascript" src="/famaotang/Public/mp/js/jquery.easyui.min.js"></script>
<script type="text/javascript" src="/famaotang/Public/mp/js/locale/easyui-lang-zh_CN.js"></script>
...
<div class="nav_l"><a href="<?php echo U('Mp/Index/index');?>">首页</a>　&gt;　<A href="<?php echo U('Mp/Orders/index');?>">订单管理</A>　&gt;　<A href="#">公司订单</A></div>
<div class="nav_r"></div>
</div>
<div class="height10"></div>
<div>
<div style="float:left; width:30%">
<form action="<?php echo U('Mp/Orders/cporders');?>"   method="post" name="fmmm"  >
...
<select name="od_state"   class="select" >
					<option value=""  <?php if($od_state == '' ): ?>selected<?php endif; ?>  >按状态过滤　</option>

					<option value="0" <?php if($od_state == '0' ): ?>selected<?php endif; ?>  >待确认</option>
					<option value="1" <?php if($od_state == '1' ): ?>selected<?php endif; ?>  >代发货</option>
					<option value="3" <?php if($od_state == '3' ): ?>selected<?php endif; ?>  >已发货</option>
					<option value="8" <?php if($od_state == '8' ): ?>selected<?php endif; ?>  >已完成</option>
				    <option value="9" <?php if($od_state == '9' ): ?>selected<?php endif; ?>  >已取消</option>
				  </select>　
<input    type="text" size="20" maxlength="20"  name="dlusername"  class="input"   onfocus="if(this.value=='请填写下单代理账号')this.value=''"   value="<?php echo ($dlusername); ?>"   style="color:#999999"  >　<input type="submit" name="Submit" value="查 询"  >
</form>
</div>
</div>
<div class="height10"></div>
...
<?php if(is_array($orderlist)): foreach($orderlist as $key=>$item): ?><tr >
<td class="data"   style="border-right:none; border-left:none; height:10px"  colspan="7"></td>
</tr>
<tr class="even" >
<td class="data" style="text-align:left; border-left:solid 1px #ddd" ><span><b>订单号：<?php echo ($item["od_orderid"]); ?></b>　　<?php echo (date('Y-m-d H:i:s',$item["od_addtime"])); ?></span></td>
...
<?php if(is_array($item['orderdetail'])): foreach($item['orderdetail'] as $key2=>$item2): ?><tr class="odd" >
<td class="data" style="text-align:left; border-left:solid 1px #ddd" ><span>
<?php if($item2["oddt_propic"] == '' ): ?><img src="/famaotang/Public/demo82/static/nophoto.jpg" style="vertical-align:middle; width:10%" />
<?php else: ?>
<img src="/famaotang/Public/uploads/product/<?php echo ($item2["oddt_propic"]); ?>" style="vertical-align:middle; width:10%" /><?php endif; ?>
　<?php echo ($item2["oddt_proname"]); ?>  (<?php echo ($item2["oddt_pronumber"]); ?>)
<?php if($item2["oddt_color"] != '' ): echo ($item2["oddt_color"]); endif; ?>　<?php if($item2["oddt_size"] != '' ): echo ($item2["oddt_size"]); endif; ?>
</span></td>
<td class="data"><span><?php if($item2["oddt_dlprice"] != '' ): ?>单价：<?php echo (number_format($item2["oddt_dlprice"],2,'.','')); ?> 元<?php endif; ?><br>订购：<?php echo ($item2["oddt_qty"]); echo ($item2["oddt_prounits"]); echo ($item2["oddt_totalqty"]); ?><br>已发：<?php echo ($item2["oddt_shipqty"]); ?></span></td>
<?php if($key2 == '0' ): ?><td class="data"  rowspan="<?php echo ($item["countdetail"]); ?>" ><span><?php echo ($item["od_dl_name"]); ?><br>(<?php echo ($item["od_dl_username"]); ?>)</span></td>
<td class="data"  rowspan="<?php echo ($item["countdetail"]); ?>" ><span><?php echo ($item["od_rcdl_name"]); if($item["od_rcdl_username"] != '' ): ?><br>(<?php echo ($item["od_rcdl_username"]); ?>)<?php endif; ?></span></td>
<td class="data"  rowspan="<?php echo ($item["countdetail"]); ?>" ><span><?php echo (number_format($item["od_total"],2,'.','')); ?> 元</span></td>
<td class="data"  rowspan="<?php echo ($item["countdetail"]); ?>" ><span><?php echo ($item["od_state_str"]); ?></span></td>
<td class="data"  rowspan="<?php echo ($item["countdetail"]); ?>" style="line-height:25px" ><span><?php echo ($item["caozuostr"]); ?></span></td><?php endif; ?>
