...
<link rel="stylesheet" type="text/css" href="/famaotang/Public/mp/css/jquery-confirm.min.css" />
<script type="text/javascript" src="/famaotang/Public/mp/js/jquery-confirm.min.js"></script>
<script type="text/javascript" src="/famaotang/Public/mp/js/jquery.form.min.js"></script>
<script type="text/javascript" language="javascript">
        $(document).ready(function() {
		
			$("#odfinishship").click(function() {
				var options = {
					url:"<?php echo U('Mp/Orders/odfinishship');?>",
					type:"post",
					dataType:"json", 
							...
							$.confirm({
								...
								buttons: {
									 confirm: {
										...
										btnClass: 'btn-blue',
										action: function () {
									         self.location.href='<?php echo U('Mp/Orders/odfinishship?od_id='.$od_id.'&odbl_id='.$odbl_id.'&isok=1');?>'; 
										}
									},
									cancel: {
										text: '取消发货',
...
<div class="nav_l"><a href="<?php echo U('Mp/Index/index');?>">首页</a>　&gt;　<A href="<?php echo U('Mp/Orders/cporders');?>">订单管理</A>　&gt;　<A href="#">订单详细</A></div>
<div class="nav_r"><?php if($back == '1' ): ?><a href="<?php echo U('Mp/Orders/cporders');?>" >&lt;&lt; 返 回</a><?php else: ?><a href="javascript:window.history.go(-1);" >&lt;&lt; 返 回</a><?php endif; ?></div>
</div>
...
<img src="/famaotang/Public/uploads/product/<?php echo ($item["oddt_propic"]); ?>" style="vertical-align:middle; width:50%" /><?php endif; ?>
    ...
    <td>&nbsp;<?php echo ($item["oddt_proname"]); ?> (<?php echo ($item["oddt_pronumber"]); ?>)　<?php if($item["oddt_color"] != '' ): echo ($item["oddt_color"]); endif; ?>　<?php if($item["oddt_size"] != '' ): echo ($item["oddt_size"]); endif; ?> <?php if($item["oddt_dlprice"] != '' ): ?>单价:<?php echo (number_format($item["oddt_dlprice"],2,'.','')); ?> 元<?php endif; ?></td>
    <td colspan="2">&nbsp; 订购：<?php echo ($item["oddt_qty"]); echo ($item["oddt_prounits"]); echo ($item["oddt_totalqty"]); ?>　已发：<?php echo ($item["oddt_shipqty"]); ?> <?php if($item["oddt_shipqty"] != '0' ): ?>　<a href="<?php echo U('./Mp/Orders/odshiplist?od_id='.$item['oddt_odid'].'&odbl_id='.$item['oddt_odblid'].'&oddt_id='.$item['oddt_id'].'&back=1');?>" style="text-decoration:underline"  >已发记录</a><?php endif; ?>　　　　　　<?php echo ($item["oddt_shipment"]); ?></td>
  ...
  <tr>
    ...
    <td colspan="3" >&nbsp;<?php echo ($ordersinfo["od_rcdl_name"]); ?> <?php if($ordersinfo["od_rcdl_username"] != '' ): ?>(<?php echo ($ordersinfo["od_rcdl_username"]); ?>)<?php endif; ?> <?php if($ordersinfo["od_rcdl_tel"] != '' ): ?>(<?php echo ($ordersinfo["od_rcdl_tel"]); ?>)<?php endif; ?></td>
        ...
        <td class="txtright txtbold"  >&nbsp;区域发货人：</td>
        <td colspan="3" >&nbsp;<?php echo ($data20); echo ($data21); ?> <?php if($ordersinfo["od_rcdl_username"] != '' ): ?>(<?php echo ($ordersinfo["od_rcdl_username"]); ?>)<?php endif; ?> <?php if($ordersinfo["od_rcdl_tel"] != '' ): ?>(<?php echo ($ordersinfo["od_rcdl_tel"]); ?>)<?php endif; ?></td>
    </tr>

  <tr>
    <td class="txtright txtbold"  >&nbsp;物流信息：</td>
    <td>
	<?php if($ordersinfo["od_expressname"] != '' ): ?>&nbsp;快递：<?php echo ($ordersinfo["od_expressname"]); ?><br><?php endif; ?>
	<?php if($ordersinfo["od_expressnum"] != '' ): ?>&nbsp;单号：<?php echo ($ordersinfo["od_expressnum"]); ?><br><?php endif; ?>
	...
	<?php if($ordersinfo["od_dealremark"] != '' ): ?>&nbsp;处理备注：<?php echo ($ordersinfo["od_dealremark"]); ?><br><?php endif; ?>
	</td>
    <td class="txtright"  >&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <?php if($odfanlilist): ?><tr>
	...
	<tbody>
	<?php if(is_array($odfanlilist)): foreach($odfanlilist as $key=>$item): ?><tr class="odd" >
	<td class="data"  ><span><?php echo ($item["dl_name_str"]); ?></span></td>
	<td class="data"  ><span><?php echo ($item["dl_sendname_str"]); ?></span></td>
	<td class="data"><span><?php echo ($item["fl_typestr"]); ?></span></td>
	<td class="data"><span><?php echo ($item["fl_moneystr"]); ?></span></td>
	<td class="data"><span><?php echo (date('Y-m-d H:i:s',$item["fl_addtime"])); ?></span></td>
	<td class="data"><span><?php echo ($item["fl_remark"]); ?></span></td>
	<td class="data"><span><?php echo ($item["fl_state_str"]); ?></span></td>
	</tr><?php endforeach; endif; ?>
	</tbody>
	</table>
...
<?php echo ($ordersinfo["caozuostr"]); ?>　　　<?php if($back == '1' ): ?><input name="back" onclick="javascript:window.self.location='<?php echo U('Mp/Orders/cporders');?>';" id="back" class="botton" value="&lt;&lt; 返 回" type="button" ><?php else: ?><input name="back" onclick="javascript:window.history.go(-1);" id="back" class="botton" value="&lt;&lt; 返 回" type="button" ><?php endif; ?> </div>
</form>
<div class="detail" >
