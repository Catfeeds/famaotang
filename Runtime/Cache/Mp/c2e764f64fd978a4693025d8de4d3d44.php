...
$(document).ready(function() {
	$("#ship_date").datebox("setValue", "<?php echo ($shipmentinfo["ship_datestr"]); ?>"); 
}); 
...
function sub_dealer(nextselect,id){
	...
	if(id>0){
		...
		$.post("<?php echo U('Mp/Shipment/subdealerarr');?>", {Action:"post",dl_id:id}, function (data, textStatus){
		   if(data.length>0){
...
<div class="nav_l"><a href="<?php echo U('Mp/Index/index');?>">首页</a>　&gt;　<A href="<?php echo U('Mp/Shipment/index');?>">出货管理</A>　&gt;　<A href="#">出货扫描</A></div>
<div class="nav_r"></div>
</div>
<div class="height20"></div>
<div class="content">
<form action="<?php echo U('Mp/Shipment/add_save');?>"   method="post" name="fmmm" >
<input type="hidden" name="ship_id" value="<?php echo ($shipmentinfo["ship_id"]); ?>" />
             ...
             <tr>     
			  ...
			  <tr>     
			   ...
			   <tr>     
				  ...
				  <td  class="txtleft" >
				  <input    type="text" size="25" maxlength="32"  name="ship_number"  class="input"  value="<?php echo ($shipmentinfo["ship_number"]); ?>"   >  * 
		      </td>
			  </tr>
			    <tr>     
				  ...
				  <select name="ship_whid"   class="select"  >
					...
					<?php if(is_array($warehouselist)): $key = 0; $__LIST__ = $warehouselist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><option value="<?php echo ($vo["wh_id"]); ?>" <?php if($shipmentinfo["ship_whid"] == $vo["wh_id"] ): ?>selected<?php endif; ?> ><?php echo ($vo["wh_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				  </select> *
		      </td>
			  </tr>
			   <tr>     
				  ...
				  <select name="ship_dealer"  id="ship_dealer"     class="select"    >
					...
					<?php if(is_array($dealerlist)): $key = 0; $__LIST__ = $dealerlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><option value="<?php echo ($vo["dl_id"]); ?>"  <?php if($shipmentinfo["ship_dealer"] == $vo["dl_id"] ): ?>selected<?php endif; ?> ><?php echo ($vo["dl_number"]); ?> <?php echo ($vo["dl_name"]); ?> (<?php echo ($vo["dl_username"]); ?>)</option><?php endforeach; endif; else: echo "" ;endif; ?>
				  </select>
				   ...
				   *
				   <?php if($shipmentinfo["old_dl_name"] != '' ): ?><br />原经销商：<?php echo ($shipmentinfo["old_dl_name"]); endif; ?>
		      </td>
			  </tr>
			   <tr>     
				  ...
				  <select name="ship_pro"   class="select" >
					...
					<?php if(is_array($productlist)): $key = 0; $__LIST__ = $productlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><option value="<?php echo ($vo["pro_id"]); ?>"  <?php if($shipmentinfo["ship_pro"] == $vo["pro_id"] ): ?>selected<?php endif; ?> ><?php echo ($vo["pro_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				  </select> *
		      </td>
			  </tr>
			   <tr>     
				  ...
				  <td  class="txtleft" >
				  <?php if($action == 'update' ): ?><span style="font-size:16px;font-weight:bold"><?php echo ($shipmentinfo["ship_barcode"]); ?></span>
				  <?php else: ?>
				  <input    type="text" size="25" maxlength="32"  name="ship_barcode"  class="input"  value=""  id="ship_barcode"   style="height:40px; line-height:40px;font-size:20px" >  *<?php endif; ?>
			  ...
			  <tr>  <td height="60" ></td>
                    <td  class="txtleft"  >
             <input type="submit" name="Submit" value="<?php echo ($mtitle); ?>" class="botton"   id="submit" >　　　<?php if($action != 'update' ): ?><input  type="reset" value="重新扫描" class="botton"  /><?php endif; ?>　　　　<input name="" type="reset" value="返 回"  onClick="javascript:window.history.go(-1);" class="botton" >
            </td>
			  </tr>
			</table>
</form>
