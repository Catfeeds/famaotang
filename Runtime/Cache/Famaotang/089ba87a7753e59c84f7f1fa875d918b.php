	...
	<body ontouchstart="" style="background-color:#FAFAFA">
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				...
				<a href="<?php echo U('./'.C('MODULE_NAME').'/Orders/dlorders/');?>" class="icon icon-left pull-left" style="color:#fff"></a>
						...
						<li>
							<div class="order-list all_odlist" style="border-bottom:.2rem solid #EFEFF4;">
								<div class="order-content" style="padding-bottom: 0.5rem">
									<div class="item-media"><img src="/famaotang/Public/uploads/product/<?php echo ($ordersinfo["oddt_propic"]); ?>" style="width:3.5rem;" onerror="this.src='/famaotang/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/logo_icon.png'"></div>
 ...
 <script type="text/javascript" src="/famaotang/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/js/sm.min.js" charset="utf-8"></script>
	...
	<script type="text/javascript">
        ...
        var oddt_id="<?php echo ($ordersinfo["oddt_id"]); ?>";
				...
				if(!isEmpty(keyword)) {
					window.location.href ="<?php echo U('./'.C('MODULE_NAME').'/Orders/odshipscanres/od_id');?>/" +od_id+"/oddt_id/"+oddt_id+"/brcode/"+keyword;
				}
			});
          ...
          if ($.isArray(codeArray)&&codeArray.length>index)
          {
              var brcode=codeArray[index];
               $(this).click(function(){
                  var od_id="<?php echo ($ordersinfo["oddt_odid"]); ?>";
                  var oddt_id="<?php echo ($ordersinfo["oddt_id"]); ?>";
                  window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/odshipremove/brcode');?>/"+brcode+"/od_id/"+od_id+"/oddt_id/"+oddt_id;
              });
          }
      });
      ...
      $('#odshipscan_sumbit').click(function() {
          ...
          if (parseInt(scancount)>0)
          {
            $.confirm('是否已核对订单信息，确定出货吗?',function () {
                var od_id="<?php echo ($ordersinfo["oddt_odid"]); ?>";
                var oddt_id="<?php echo ($ordersinfo["oddt_id"]); ?>";
                window.location.href ="<?php echo U('./'.C('MODULE_NAME').'/Orders/odshipping/od_id');?>/" +od_id+"/oddt_id/"+oddt_id+"/step/1";
            });
          }else
          {
            $.toast('请扫描条码再出货！');
  ...
  // 1 判断当前版本是否支持指定 JS 接口，支持批量判断
    ...
    $('#scanQRCode1').click(function() {
	    ...
	    wx.scanQRCode({
	        ...
	        self.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/odshipscanres/od_id');?>/"+od_id+"/oddt_id/"+oddt_id+"/brcode/"+result; 
	      	}
	    });
	});
  });
