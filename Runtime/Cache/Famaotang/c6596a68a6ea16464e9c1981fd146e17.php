	...
	<body ontouchstart="" style="background-color:#FAFAFA">
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				...
				<?php else: ?>
					<a href="<?php echo U('./'.C('MODULE_NAME').'/Dealer');?>" class="icon icon-left pull-left" style="color:#fff"></a><?php endif; ?>
			...
			$('.order-list.dl_search').each(function(index) {
				...
				$(this).click(function(){
					if ($.isArray(dlsArray)&&dlsArray.length>index)
					{
						...
						{
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Dealer/dealerdetail/dlid');?>/"+dlsObject['dl_id'];
						}
					}
				});
			});
				...
				$(this).click(function(){
					$.confirm('确定删除该代理吗?', function () {
        				...
        				if ($.isPlainObject(dldObject))
						{
							...
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Dealer/applydelete/dlid');?>/"+dldObject['dl_id'];
						}
            		});
				});
			});
			...
			$('.dl_confirm').each(function(index) {
				...
				$(this).click(function(){
					$.confirm('确定通过该代理吗?', function () {
						...
						{
							// $.toast("确定删除"+dldObject['dl_id']);
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Dealer/applyactive/dlid');?>/"+dlcObject['dl_id'];
						}
            		});
				});
			});
