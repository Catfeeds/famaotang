		...
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				...
				<?php else: ?>
					...
					<?php else: ?>
						<a href="<?php echo U('./'.C('MODULE_NAME').'/Orders/home/od_type/0');?>" class="icon icon-left pull-left" style="color:#fff"></a><?php endif; endif; ?>
				<h1 class="title" style="color:#fff">我的订单</h1>
			</header>
			<div class="content">
													...
													<div class="order-list all_odlist" style="border-bottom:.2rem solid #EFEFF4;">
														...
														<div class="order-bottom">
															<div class="order-button-type">
																<?php if($item["od_state"] == 0): ?><a href="#" class="button button-light button-round order-button" id="all_odcancel" style="color:#ccc;margin-left:.5rem;font-size: 0.5rem;">取消</a><?php endif; ?>
																<?php if($item["od_state"] == 3): ?><a href="#" class="button button-warning button-round order-button" id="all_odshouhuo" style="margin-left:.5rem;font-size: 0.5rem;">收货</a><?php endif; ?>
																<?php if($item["od_state"] < 3): ?><a href="#" class="button button-warning button-round order-button" id="all_odpingzheng" style="margin-left:.5rem;font-size: 0.5rem;">凭证</a><?php endif; ?>
			...
			$('.order-list.all_odlist').each(function(index) {
				...
				if ($.isArray(odArray)&&odArray.length>index)
				{
					...
					$(this).find("#all_oddetail").click(function(){
						...
						{
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/orderdetail/od_id');?>/"+odObject['od_id'];
						}
					});
					$(this).find("#all_odqueren").click(function(){
						if ($.isPlainObject(odObject))
						{
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/cancelmyorder/od_id');?>/"+odObject['od_id']+"/state/1";
						}
					});
