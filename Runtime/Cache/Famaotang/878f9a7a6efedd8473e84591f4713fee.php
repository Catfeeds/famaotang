				            ...
				            <div class="kl-layout-horizontally-between" style="width: 100%;margin-bottom: 0.2rem; background: #fff">
				            	<div class="kl-layout-horizontally-between" style="width: 93%;">
	              				    <div class="item-content item-inner">
	            						...
	            						<div class="item-title">向 <?php if(!empty($item["fl_sdl_name"])): echo ($item["fl_sdl_name"]); endif; ?> <?php if(!empty($item["fl_sdl_username"])): ?>(<?php echo ($item["fl_sdl_username"]); ?>)<?php endif; ?> <?php echo ($item["rc_str"]); ?></div>
	          						</div>
					 				<p><a href="#" class="button button-fill button-warning button-round" style="padding:0 0.6rem;font-size: .5rem;"><?php echo ($item["rc_statestr"]); ?></a></p>
				           		</div>
            ...
            $('.paylist_item').each(function(index) {
                ...
                if ($.isArray(recArray)&&recArray.length>index)
                {
                    ...
                    $(this).click(function(){
                        ...
                        {
                            window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Fanli/recashdetail');?>/"+"rc_id/"+recObject['rc_id'];
                        }
                    }); 
                }
            });
