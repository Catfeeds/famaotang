	...
	<body ontouchstart="" style="background-color:#FAFAFA">
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
                <?php if($back == 2): ?><a href="<?php echo U('./'.C('MODULE_NAME').'/Mine');?>" class="icon icon-left pull-left" style="color:#fff"></a>
				<?php else: ?>
				<a href="<?php echo U('./'.C('MODULE_NAME').'/Index');?>" class="icon icon-left pull-left" style="color:#fff"></a><?php endif; ?>
						...
						<ul class="kl-products-list">
							<?php if(is_array($list)): foreach($list as $key=>$item): ?><li class="kl-products-item">
								...
								<div style="padding:0 .8rem .2rem .8rem">
									...
									<?php if(($item["pro_price"] != '') AND ($item["pro_price"] != '0') ): ?><span style="padding-right:.2rem; <?php if($login ): ?>text-decoration:line-through<?php endif; ?>">￥<?php echo (number_format($item["pro_price"],2,'.','')); ?></span><?php endif; ?>
								</div>
	...
	$(function() {
		...
		$('.kl-products-item').each(function(index) {
			...
			$(this).click(function(){
				if ($.isArray(proArray)&&proArray.length>index)
				{
					...
					{
						window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Product/detail/pro_id');?>/"+proObject['pro_id']+"";
					}
				}
			});
		});
		...
		$('#pro_order').click(function() {
				if (order_status==1)
				{
					...
					window.location.href = "<?php echo U('./'.C('MODULE_NAME').'/Product/index/order_status/');?>/0"+"";
				}
				else
				{
					window.location.href = "<?php echo U('./'.C('MODULE_NAME').'/Product/index/order_status/');?>/1"+"";
				}
		});
		...
		var idTypeArry=[];
			...
			$('#pro_filter').picker({
				...
				onClose:function(e)
				{
					location.href = "<?php echo U('./'.C('MODULE_NAME').'/Product/index/order_status/');?>/"+order_status+"/protype_id/"+protype_id+"";;
				}
			});
