  ...
  <body ontouchstart="" style="background-color:#FAFAFA">
    <div class="page">
      <header class="bar bar-nav" style="background-color:#006db2;">
		...
		<a href="<?php echo U('./'.C('MODULE_NAME').'/Orders/region_order/od_state/'.$ordersinfo['od_state'].'');?>" class="icon icon-left pull-left" style="color:#fff"></a>
        <h1 class="title" style="color:#fff">下家订单详情</h1>
      </header>
      <div class="content">
            ...
            <li>
              <div class="order-list all_odlist">
                <?php if(is_array($ordersinfo['orderdetail'])): foreach($ordersinfo['orderdetail'] as $key=>$item): ?><div class="order-content" style="padding-bottom: 0.2rem">
                    <div class="item-media"><img src="/famaotang/Public/uploads/product/<?php echo ($item["oddt_propic"]); ?>" style="width:3.5rem;" onerror="this.src='/famaotang/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/logo_icon.png'"></div>
                      ...
                      <div class="order-remark">
                        <div class="kl-layout-horizontally-between">
                          <div>
                            ...
                            <div class="pro_name">订购：<?php echo ($item["oddt_qty"]); echo ($item["oddt_prounits"]); echo ($item["oddt_totalqty"]); ?>　已发：<?php echo ($item["oddt_shipqty"]); ?>
									<?php if($item["oddt_shipqty"] > 0): ?><i class="iconfont icon-shenqing all_recode_icon" style="font-size:1rem; color: #7e7e7e;margin-left:0.25rem"></i><?php endif; ?>
                            </div>
                          </div>
                          <!--<?php if(($item["oddt_shipment"] == 1)): ?>-->
                              <!--<i class="iconfont icon-saoyisao all_odsaomian" style="font-size: 2rem;color: #167abe;"></i>-->
                          <!--<?php endif; ?>-->
                        </div>
                      </div>
                ...
                <div class="order-bottom" <?php if($ordersinfo["od_state"] > 7): ?>style="display:none"<?php endif; ?> >
                  <div class="order-button-type">
                    <?php if($ordersinfo["od_state"] < 3): ?><a href="#" class="button button-light button-round order-button" id="dtl_odcancel" style="color:#ccc;margin-left:.5rem; border-radius:0.2rem;font-size: 0.5rem;">取消订单</a><?php endif; ?>
                    <?php if($ordersinfo["od_state"] == 0 ): ?><a href="#" class="button button-warning button-round order-button" id="dtl_odqueren" style="margin-left:.5rem;border-radius:0.2rem;font-size: 0.5rem;">确认订单</a><?php endif; ?>
                    <?php if(($ordersinfo["od_state"] < 3) and ($ordersinfo["od_state"] > 0)): ?><a href="#" class="button button-warning button-round order-button" id="dtl_finish" style="margin-left:.5rem;border-radius:0.2rem;font-size: 0.5rem;">完成发货</a><?php endif; ?>
                  </div>
                </div>
              </div>
            </li>
  ...
  <script type="text/javascript">
    ...
    $(function() {
        ...
        $("#dtl_odqueren").click(function(){
            if (od_id>0)
              window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/cancelquyorder/od_id');?>/"+od_id+"/state/1/od_state/"+od_state;
          });
        $("#dtl_odcancel").click(function(){
            if (od_id>0){
                $.confirm('是否确定取消该订单吗?',function () {
					 window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/cancelquyorder/od_id');?>/"+od_id+"/state/9/od_state/"+od_state;
				});
        ...
        $("#dtl_finish").click(function(){
            if (od_id>0)
            window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/quyodfinishship/od_id');?>/"+od_id+"/state/9/od_state/"+od_state;
        });
        ...
        if($.isArray(proArray)&&proArray.length>0){
           $(".all_recode_icon").each(function(index){
            ...
            $(this).click(function(){
              ...
              {
                window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/odshiplist/od_id');?>/"+proObject['oddt_odid']+"/oddt_id/"+proObject['oddt_id']+"/isdetail/1";
              }
            });
          });
          ...
          $(".all_odsaomian").each(function(index){
            ...
            $(this).click(function(){
              if ($.isPlainObject(proObject))
              {
                window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/odshipscan/od_id');?>/"+proObject['oddt_odid']+"/oddt_id/"+proObject['oddt_id'];
              }
            });
          });  
        }
