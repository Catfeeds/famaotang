 ...
 <script type="text/javascript" src="/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/js/sm.min.js" charset="utf-8"></script>
    ...
    $("#feedback").click(function(){
            ...
            try {
				$.ajax({
					type: 'POST',
					url: '<?php echo U('./'.C('MODULE_NAME').'/Feedback/index');?>',
					...
					success: function (data) {
						...
						if (stat == 1) {
							$.toast(data.msg);
						     location.href = "<?php echo U('./'.C('MODULE_NAME').'/Mine');?>";
						}else if(stat == 2){
							$.toast(data.msg);
							 location.href = "<?php echo U('./'.C('MODULE_NAME').'/index');?>";
						}else{
							$.toast(data.msg);
							 return false; 
						}
    	...
    	$("#verifyImg").click(function(){
			$("#verifyImg").attr("src","<?php echo U('./'.C('MODULE_NAME').'/Feedback/verify');?>?"+ Math.random());
		});
	});
	</script>	
</html>