		...
		<div class="page">
			...
			<div class="content">
				<div class="kl-layout-login" style="height:10rem;">
					<img src="/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/fw_icon.png" style="width:5rem;height:5rem;line-height:5rem;margin-top:2rem;">
				</div>
				<form action="#">
						...
						<ul>
							...
							<li id="checkcode_li" <?php if($is_checkcode == 0): ?>style="display:none"<?php endif; ?>>
								<div class="item-content">
									...
									<img alt="点击换另一个" title="点击换另一个" style="width:6rem;vertical-align:middle; cursor:pointer" id="verifyImg" src="<?php echo U('./'.C('MODULE_NAME').'/Query/verify');?>" />
								</div>
							</li>
						</ul>
					</div>
				...
				<div class="card">
					...
					<div class="card-footer" id="guanzhu" <?php if($msg == '' ): ?>style="display:none"<?php endif; ?>><p style="text-align:center;margin: 0 auto"><img src="/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/weixin.jpg" style="width:40%"><br>长按二维码关注</p></div>
				</div>
			</div>
		</div>
	</body>
	...
	<script type="text/javascript">
		...
		$(function() {
			$("#verifyImg").click(function() {
				$("#verifyImg").attr("src", "<?php echo U('./'.C('MODULE_NAME').'/Query/verify');?>?" + Math.random());
			});
			...
			$("#search_sumbit").click(function() {
				...
				try {
					$.ajax({
						type: 'POST',
						url: '<?php echo U('./'.C('MODULE_NAME').'/Query/ajaxres');?>',
						data: {
							fwcode: fwcode,
							checkcode: checkcode,
							referer: referer
