...
<html>
	...
	<head>
		...
		<title><?php echo ($dlt_name); ?> 代理团队</title>
	...
	<body ontouchstart="" style="background-color:#FAFAFA">
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				<a href="<?php echo U('./'.C('MODULE_NAME').'/Dealer/index');?>" class="icon icon-left pull-left" style="color:#fff"></a>
			...
			<div class="content">
				<div class="list-block media-list" style="line-height:1rem; margin-top:.25rem;margin-bottom: 0rem;">
									<ul>
										<?php if(is_array($dltlist)): foreach($dltlist as $key=>$item): ?><li>
												<div class="order-list dl_tuandui" style="border-bottom:.2rem solid #EFEFF4;">
													<div class="order-content">
														<div class="item-media"><img class="kl-circle kl-img-thumbnail" src="/Public/uploads/mobi/<?php echo ($item["dl_wxheadimg"]); ?>" style="width:2.5rem; border-radius: 50%;" onerror="this.src='/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/head_icon.png'"></div>
														<div class="order-inner">
															<div class="item-subtitle between-horizontally"><span><?php echo ($item["dl_name"]); echo ($item["dl_status_str"]); ?></span><span style="color:#c1c1c1"><?php echo (date('Y-m-d h:m',$item["dl_addtime"])); ?></span></div>
															<div class="order-remark">
																<div class="item-title between-horizontally" style="color:#c1c1c1"><div>级别：<span style="color:red"><?php echo ($dlt_name); ?></span> 代理团队</div><span style="color:#c1c1c1">上级：<?php echo ($item["dl_belong_str"]); ?></span></div>
        		...
        		$(this).click(function(){
					if ($.isArray(dltArray)&&dltArray.length>index)
					{
						...
						if ($.isPlainObject(dltObject))
						{
							...
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Dealer/dealerdetail/dlid');?>/"+dltObject['dl_id'];
						}
					}
				});
			});
