	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#dl_tiaoji').click(function() {
				location.href = "<?php echo U('./'.C('MODULE_NAME').'/Dealer/updatedltypeindex/up_status/0');?>";
			});
			$('#dl_tuijian').click(function() {
				location.href = "<?php echo U('./'.C('MODULE_NAME').'/Dealer/referee');?>";
			});
						...
						if ($.isPlainObject(dltObject))
						{
							$.each(dltObject, function(key, val) {
								if(key=="count"&&val>0) {
									 ...
									 //window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Dealer/refereelist/dltid');?>/"+dltObject['dlt_id'];
    							}
    						});
						}
