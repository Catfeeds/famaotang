		...
		$(function() {
			...
			$("#address_del").click(function() {
				$.confirm('确定删除该地址吗?', function () {
					...
					window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/deleteaddress/addid');?>/"+addid+"/fromod/"+fromod;
            	});
			});
			//点击提交
			$("#save_address").click(function() {
				...
				if($("#dl_address").val() == $("#dl_area_all").val()) {
					$.toast("请补充填写详细收货地址");
					return false;
				}
				...
				var dladd_address = $("#dl_address").val();
				...
				try {
					$.ajax({
						type: 'POST',
						url: '<?php echo U('./'.C('MODULE_NAME').'/Orders/saveaddress');?>',
						data: {
							dladd_id:dladd_id,
						...
						success: function(data) {
							...
							if(stat == 1) {
								location.href ="<?php echo U('./'.C('MODULE_NAME').'/Orders/dladdress/dladd_id');?>/"+dladd_id+"/fromod/"+fromod;
								return false;
							} else if(stat == 2) {
								$.toast(data.msg);
								// location.href = "<?php echo U('./'.C('MODULE_NAME').'/Dealer/login');?>";
								return false;
							} else {
								$.toast(data.msg);
								return false;
