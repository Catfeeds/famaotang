...
<link rel="stylesheet" type="text/css" href="/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/css/demo.css"/>
		...
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				<!-- <a href="javascript:;" onClick="javascript :history.back(-1);" class="icon icon-left pull-left" style="color:#fff"></a> -->
				 <a href="<?php echo U('./'.C('MODULE_NAME').'/Dealer/updatedltypeindex');?>" class="icon icon-left pull-left" style="color:#fff"></a>
				<h1 class="title" style="color:#fff">申请调级</h1>
			</header>
			<div class="content">
			<form action="<?php echo U('./'.C('MODULE_NAME').'/Dealer/updatedltype_save');?>" action=""  method="post" id="fmmm" name="fmmm" >
						...
						<ul style="background: transparent;">
							...
							<li style="background:#fff;">
								<div class="item-content">
									<div class="item-media"><i class="icon icon-form-name"></i></div>
									<div class="item-inner">
										<img class="kl-img-thumbnail" id="img_file" src="" style="width:4rem;display: none;">
										<div class="item-input">	  
 ...
 <script type="text/javascript" src="/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/js/sm.min.js" charset="utf-8"></script>
	...
	<script type="text/javascript" src="/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/js/lrz.all.bundle.js"></script>
	<script type="text/javascript">
	$.init();
	$(function() {
			var listArray = <?php echo json_encode($dltypelist,true);?>||[]; //注意，这里不要用双引号或单引号；
			var nTypeArry=[];
			var idTypeArry=[];
			...
			$('#dl_level').picker({
				...
				onClose:function(e)
				{
					...
					$.ajax({
					type: 'POST',
					url: '<?php echo U('./'.C('MODULE_NAME').'/Dealer/belongjson');?>',
					data: {dltype:dltype},
					dataType: 'json',
					timeout: 30000,
					success: function (data) {
						...
						if (stat == 1) {
							 ...
							 $("#okk").val('0');
							 $.toast(data.msg);
							 // location.href = "<?php echo U('./'.C('MODULE_NAME').'/Index/index');?>";
						}else{
						     $("#belongtxt").html("");
							 $("#dl_belong").hide();
							 $("#okk").val('0');
