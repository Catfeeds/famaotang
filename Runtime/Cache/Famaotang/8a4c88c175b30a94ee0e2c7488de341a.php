				...
				<div class="list-block media-list">
					<ul class="list-container">
						<?php if(is_array($list)): foreach($list as $key=>$item): ?><li>
								<a href="<?php echo U('./'.C('MODULE_NAME').'/News/detail?news_id='.$item['news_id'].'');?>" class="item-link item-content">
									<div class="item-media"><img src="/Public/uploads/mobi/<?php echo ($item["news_pic"]); ?>" style="width:3rem;" onerror="this.src='/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/logo_icon.png'"></div>
		...
		$(function() {
			...
			$(document).on('refresh', '.pull-to-refresh-content', function(e) {
				...
				try {
					$.ajax({
						type: 'POST',
						url: '<?php echo U('./'.C('MODULE_NAME').'/News/getNewList');?>',
						data: {
							action: 'newlist',
							page: pageNum
						},
							...
							if(stat == 1) {
								setTimeout(function() {
									...
									if($.isArray(listArray) && listArray.length > 0) {
										...
										var itemHtml = '';
										var imagedetaul = '\'/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/logo_icon.png\'';
										// console.log(imagedetaul);
										listArray.forEach(function(val, index) {
											itemHtml += '<li><a href="<?php echo U('./'.C('MODULE_NAME').'/News/detail/news_id');?>/' + val.news_id + '" class="item-link item-content"><div class="item-media"><img src="/Public/uploads/mobi/' + val.news_pic + '" style="width:3rem;" onerror="this.src=' + imagedetaul + '"></div><div class="item-inner"><div class="item-title-row"><div class="item-subtitle" style="color:#7e7e7e">' + val.news_title + '</div><div class="item-after" style="font-size:0.3rem;color:#c1c1c1">' + formatDate(val.news_addtime) + '</div></div><div class="item-text" style="font-size:0.5rem;color:#c1c1c1">' + val.news_content_s + '</div></div></a></li>';
										});
