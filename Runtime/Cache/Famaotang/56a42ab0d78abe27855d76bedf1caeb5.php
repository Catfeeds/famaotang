...
<link rel="stylesheet" type="text/css" href="/famaotang/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/css/demo.css"/>
	...
	<body ontouchstart="" style="background-color:#FAFAFA">
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				<?php if($odoneself > 0): ?><a href="<?php echo U('./'.C('MODULE_NAME').'/Orders/index/ly_status/1');?>" class="icon icon-left pull-left" style="color:#fff"></a>
				<?php else: ?>
 ...
 <script type="text/javascript" src="/famaotang/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/js/sm.min.js" charset="utf-8"></script>
	...
	<script type="text/javascript">
		$.init();
		$(function() {
			var expressArray = <?php echo json_encode($expresslist);?>||[]; //注意，这里不要用双引号或单引号；
    	...
    	$("#finishship_sumbit").click(function(){
            ...
            try {
				$.ajax({
					type: 'POST',
					url: '<?php echo U('./'.C('MODULE_NAME').'/Orders/quyodfinishship');?>',
					...
					success: function (data) {
						...
						if (stat == 1) {
							  // alert(data.msg);
							 $.toast(data.msg);
							 // location.href = "<?php echo U('./'.C('MODULE_NAME').'/Orders/dlorderdetail?od_id='.$od_id.'&odbl_id='.$odbl_id.'&od_state='.$od_state.'');?>";
							location.href = "<?php echo U('./'.C('MODULE_NAME').'/Orders/region_order?od_id='.$od_id.'&odbl_id='.$odbl_id.'&od_state='.$od_state.'');?>";
							 return false; 
						}else{
							$.toast(data.msg);
