	...
	<body ontouchstart="" style="background-color:#FAFAFA">
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				...
				<?php if($dladd_id > 0): ?><a href="<?php echo U('./'.C('MODULE_NAME').'/Orders/checkshopcart/');?>/dladd_id/<?php echo ($dladd_id); ?>" class="icon icon-left pull-left" style="color:#fff"></a>
				<?php else: ?>
					<a href="<?php echo U('./'.C('MODULE_NAME').'/Mine');?>" class="icon icon-left pull-left" style="color:#fff"></a><?php endif; ?>
		...
		$(function() {
			...
			$('.edit_icon').each(function(index) {
				...
				if ($.isArray(adArray)&&adArray.length>index)
				{
					...
					{
						window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/updateaddress/fromod');?>/"+fromod+"/addid/"+adObject['dladd_id'];
					}    		
				});
				}
			});
			...
			$('.kl-layout-horizontally-vcenter.edit_address').each(function(index) {
				...
				if ($.isArray(adArray)&&adArray.length>index)
				{
					...
					$(this).click(function(){
						...
						{
							if (fromod==1)
								window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/checkshopcart/dladd_id/');?>/"+adObject['dladd_id'];
							else
								window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/defaultaddress/addid');?>/"+adObject['dladd_id'];
						}
					});
				}
			});
			...
			$("#add_address").click(function() {
				window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/updateaddress/fromod');?>/"+fromod;
			});
		});
	</script>
