		...
		<div class="page">
			<header class="bar bar-nav" style="background-color:#006db2;">
				...
				<?php else: ?>
					...
					<?php else: ?>
						<a href="<?php echo U('./'.C('MODULE_NAME').'/Orders/home/od_type');?>" class="icon icon-left pull-left" style="color:#fff"></a><?php endif; endif; ?>
				<h1 class="title" style="color:#fff">区域发货订单</h1>
			</header>
			<div class="content">
				...
				<div class="content-block" style="min-height: 10rem; margin:0;">
					<div class="buttons-tab">
						...
						<a href="#" id="od_send" class="tab-link button <?php if($od_state == 1 ): ?>active<?php endif; ?>">待发货</span></a>
											...
											<?php if(is_array($list)): foreach($list as $key=>$item): ?><li>
													<div class="order-list all_odlist" style="border-bottom:.2rem solid #EFEFF4;">
														...
														<?php if(is_array($item['orderdetail'])): foreach($item['orderdetail'] as $key2=>$item2): ?><div class="order-content" style="padding-bottom: 0.5rem">
																<div class="item-media"><img src="/famaotang/Public/uploads/product/<?php echo ($item2["oddt_propic"]); ?>" style="width:3.5rem;" onerror="this.src='/famaotang/Public/<?php echo C('RESOURCE_ROOT_NAME');?>/static/logo_icon.png'"></div>
																<div class="order-inner">
																	...
																	<div class="order-remark">
																		<div class="kl-layout-horizontally-between">
                                                                            ...
                                                                            <!--<?php if(($item["od_state"] == 1 or $item["od_state"] == 2 or $item["od_state"] == 3 ) and ($item2["oddt_canship"] == 1) ): ?>-->
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('.order-list.all_odlist').each(function(index) {
				...
				if ($.isArray(odArray)&&odArray.length>index)
				{
					...
					$(this).find("#all_oddetail").click(function(){
						...
						{
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/quyorderdetail/od_id');?>/"+odObject['od_id'];
						}
					});
					//确认
					$(this).find("#all_odqueren").click(function(){
						if ($.isPlainObject(odObject))
						{
							window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/cancelquyorder/od_id');?>/"+odObject['od_id']+"/state/1";
						}
					});
					...
					$(this).find("#all_odcancel").click(function(){
						if ($.isPlainObject(odObject))
						{
							 $.confirm('是否确定取消该订单吗?',function () {
				               window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/cancelquyorder/od_id');?>/"+odObject['od_id']+"/state/9";
				            });
						}
					});
					//扫码
					...
					$(this).find("#all_odfinish").click(function(){
						if ($.isPlainObject(odObject))
						{
								window.location.href="<?php echo U('./'.C('MODULE_NAME').'/Orders/quyodfinishship/od_id');?>/"+odObject['od_id'];
						}
					});
			...
			$('#od_sure').click(function(){
				window.location.href= "<?php echo U('./'.C('MODULE_NAME').'/Orders/region_order/od_state/0');?>";
			});

			$('#od_send').click(function(){
				window.location.href= "<?php echo U('./'.C('MODULE_NAME').'/Orders/region_order/od_state/1');?>";
			});

			$('#od_sended').click(function(){
				window.location.href= "<?php echo U('./'.C('MODULE_NAME').'/Orders/region_order/od_state/3');?>";
			});

			$('#od_finish').click(function(){
				window.location.href= "<?php echo U('./'.C('MODULE_NAME').'/Orders/region_order/od_state/8');?>";
			});

			$('#od_cancel').click(function(){
				window.location.href= "<?php echo U('./'.C('MODULE_NAME').'/Orders/region_order/od_state/9');?>";
			});
