...
//生成二维码 png
function make_ercode($codelink='',$img_file='',$img_file_tmp='',$colorarr=array(),$gg='M',$ss=4){   
    import("Org.Util.QRcode.QRcode");
	if(count($colorarr)==4){
	    QRcode::png($codelink, $img_file_tmp, $gg, $ss, 2,true);
		make_colorimg($img_file_tmp,400,400,$img_file,$colorarr);
		@unlink($img_file_tmp);
	}else{
	    QRcode::png($codelink, $img_file, $gg, $ss, 2,true); //$codelink-二维码内容 $img_file-保存路径 $gg-容错级别(L,M,Q,H) $ss-二维码大小 2-周围边框空白区  true-是否保存
	}	
}
//生成二维码 png +彩色
