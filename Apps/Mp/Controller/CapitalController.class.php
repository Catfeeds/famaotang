...
class CapitalController extends CommController {
    ...
    public function index(){
		...
		$map['dl_level']=array('in','1,2,3,4'); //只前4级
        $Dealer = M('Dealer');
		$Dltype = M('Dltype');
		$Yufukuan= M('Yufukuan');
		$Balance= M('Balance');
		...
		$map2=array();
        $map2['dlt_unitcode']=session('unitcode');
		$map2['dlt_level']=array('in','1,2,3,4');
    ...
    public function dlbalance(){
		...
		$begintime=I('param.begintime','');
        $endtime=I('param.endtime','');
		$bl_type=intval(I('param.bl_type',0));
		...
		if($bl_type==1){
			$map['bl_type']=1;
			$map['bl_receiveid']=array('GT',0);
		}else if($bl_type==2){
			$map['bl_type']=1;
			$map['bl_sendid']=array('GT',0);
		}else if($bl_type==3){
			$map['bl_type']=2;
		}else if($bl_type==4){
			$map['bl_type']=3;
		...
		$parameter['bl_type']=$bl_type;
		$this->assign('bl_type', $bl_type);
    ...
    public function balancedetail(){
		...
		if($data){
			...
			$flarr=array();
			//充值相关返利
			if($data['bl_type']==1 && $data['bl_receiveid']>0){
				$Fanlidetail= M('Fanlidetail');
				$map2=array();
				$map2['fl_unitcode']=session('unitcode');
				$map2['fl_blid']=$bl_id;

				$flarr = $Fanlidetail->where($map2)->order('fl_addtime DESC')->limit(20)->select();
		...
		$this->assign('flarr', $flarr);
        $this->assign('curr', 'dlbalance');
        $this->display('balancedetail');
	}
	/*
	//手动返临时使用
	public function balancedetail_fanli(){
		$this->check_qypurview('18004',1);
		...
		$Balance= M('Balance');
		$Dealer= M('Dealer');
		$data1=$Balance->where($map)->find();
		if($data1){
			if($data1['bl_type']==1 && $data1['bl_receiveid']>0){
				$map=array();
				$map['dl_id']=$data1['bl_receiveid'];
				$map['dl_unitcode']=session('unitcode');
				$map['dl_status']=1;
				$data=$Dealer->where($map)->find();
				if($data){
					$dl_type=$data['dl_type'];  //代理商级别id
					$dl_level=$data['dl_level'];  //代理商级别
					$dl_referee=$data['dl_referee'];//推荐人
					$dl_name=$data['dl_name'];//推荐人
					$dl_username=$data['dl_username'];//推荐人
				}else{
					$this->error('没有代理记录','',2);
				}
				
				$bl_money=$data1['bl_money'];
				$addtime=$data1['bl_addtime'];
				$result=$bl_id;
				if($dl_level==2 ||$dl_level==3 || $dl_level==4){
					$fanli_dlid1=0;
					$fanli_dlname1='';
					$fanli_dlusername1='';
					$fanli_dlid2=0;
					...
					$map4['dl_id'] = $dl_referee;  //推荐人
					$map4['dl_status'] = 1;
					$data4=$Dealer->where($map4)->find();
					if($data4){
						if($data4['dl_id']>0){
							if($data4['dl_level']==1 || $data4['dl_level']==2 || $data4['dl_level']==3 || $data4['dl_level']==4){
								...
								if($data4['dl_referee']>0){
									...
									if($data6){
										if($data6['dl_level']==1 || $data6['dl_level']==2 || $data6['dl_level']==3 || $data6['dl_level']==4){
					...
					if($fanli_dlid1>0){
						...
						$map22=array();
						$map22['pri_proid']=1; 
						$map22['pri_unitcode']=session('unitcode');
						$map22['pri_dltype'] = $dl_type;
						$data22=$Proprice->where($map22)->find();
						if($data22){
							$pri_price=$data22['pri_price'];  //
						}else{
							$pri_price=0;
						}
						if($pri_price>0){
							//计算组数
							$zushu=floor($bl_money/$pri_price);
								...
								if($data7['pfl_fanli1']>0){
									...
									if(!$data8){
										...
										if($data7['pfl_fanli1']>0 && $data7['pfl_fanli1']<1){
											$pfl_fanli1sum=$data7['pfl_fanli1']*$bl_money;
										}else{
											$pfl_fanli1sum=$data7['pfl_fanli1']*$zushu;
										}
										...
										$data5['fl_blid']  = $result; //充值明细的id
										
										$data5['fl_addtime']  = $addtime;
										$data5['fl_remark'] ='代理 '.$dl_name.'('.$dl_username.') 充值返 ' ;
									...
									if($data7['pfl_fanli2']>0){
										...
										if(!$data8){
											...
											if($data7['pfl_fanli2']>0 && $data7['pfl_fanli2']<1){
												$pfl_fanli2sum=$data7['pfl_fanli2']*$bl_money;
											}else{
												$pfl_fanli2sum=$data7['pfl_fanli2']*$zushu;
											}
											...
											$data5['fl_blid']  = $result; //充值明细的id
											$data5['fl_addtime']  = $addtime;
											$data5['fl_remark'] ='代理 '.$fanli_dlname1.'('.$fanli_dlusername1.') 的邀请代理 '.$dl_name.'('.$dl_username.') 充值返' ;
											$rs5=$Fanlidetail->create($data5,1);
											if($rs5){
												$Fanlidetail->add();
											}
		...
		$this->success('操作完成',U('Mp/Capital/balancedetail/bl_id/'.$bl_id.''));
	}
	*/
    ...
    public function yueadd_save(){
		...
		if($dl_id>0){
			...
			$data['bl_orderid']='';
			$data['bl_odblid']=0;
			$data['bl_addtime']=$addtime;
			$data['bl_remark']=$bl_remark;
			$data['bl_state']=1;
			$data['bl_rcid']=0;
			$data['bl_dltype']=$dl_type; //充值时的代理级别
    ...
    public function payindetail(){
		...
		if($data){
			...
			if(is_not_null($data['pi_pic2']) && file_exists($imgpath.$data['pi_pic2'])){
                $data['pi_pic2_str']='<img src="'.__ROOT__.'/Public/uploads/dealer/'.$data['pi_pic2'].'"   border="0" style="vertical-align:middle;width:10%"  >';
            }else{
                $data['pi_pic2_str']='';
            }
