...
class DealerController extends CommController {
    ...
    public function index(){
		...
		//所推荐的
		$dl_refid=intval(I('param.dl_refid',0));
	    if($dl_refid>0){
	    	$map['dl_referee']=$dl_refid;
			$parameter['dl_refid']=$dl_refid;
	    }
        ...
        $show = $Page->show();
        $list = $Dealer->where($map)->order('dl_id DESC')->limit(($Page->firstRow . ',') . $Page->listRows)->select();
		...
		foreach($list as $k=>$v){
			...
			if($v['dl_referee']>0){
				...
				$data2 = $Dealer->where($map2)->find();
				if($data2){
                    $list[$k]['dl_referee_str']=$data2['dl_name'].'';
				}else{
    ...
    public function tree(){
		...
		//架构树缓存
		$dealertreelisthtml = S('dealertreelisthtml'.session('unitcode')); //缓存
		if($dealertreelisthtml){
			$this->list_html=$dealertreelisthtml;
		}else{
			$map['dl_belong']=0;
			$map['dl_unitcode']=session('unitcode');
			$Dealer = M('Dealer');
			...
			foreach($list as $k=>$v){	
				...
				//所有下级总数
				if($count3>0){
					$countall=$this->treesubcount($v['dl_id']);
				}
				$list[$k]['dl_countall']=$countall;
			...
			$map3['dl_belong']=0;
			$map3['dl_unitcode']=session('unitcode');
			$gszscount = $Dealer->where($map3)->count();
			
			//公司全部
			$map3=array();
			$map3['dl_unitcode']=session('unitcode');
			$gsallcount = $Dealer->where($map3)->count();
			
			
			$this->list_html.='<li>';
			$this->list_html.='<img  src="'.__ROOT__.'/Public/mp/static/menu_minus.gif"   /> 公司 (直:'.$gszscount.' 总:'.$gsallcount.')';
			$this->list_html.='<ul  class="treeitem" style="display: block;" >';
			
			foreach($list as $k=>$v){
				$this->list_html.='<li>';
				//下线记录
				if($v['dl_subcount']>0){
					$this->list_html.='<img  src="'.__ROOT__.'/Public/mp/static/menu_plus.gif"  onClick="toggleCollapse(\'#s'.$v['dl_id'].'\',this)"   /> <a href="'.U('Mp/Dealer/treedetail?dl_id='.$v['dl_id']).'"  target="dealerframe"  >'.htmlspecialchars($v['dl_name']).'('.htmlspecialchars($v['dl_username']).')</a>'.htmlspecialchars($v['dl_type_str']).$v['dl_status_str'].' (直:'.$v['dl_subcount'].' 总:'.$v['dl_countall'].')';
			...
			//写入缓存
			S('dealertreelisthtml'.session('unitcode'),$this->list_html,array('type'=>'file','expire'=>900));
		}
   ...
   //经销商架构树递归
    ...
    //下级代理 总数 递归
    public function treesubcount($dlid){
		$countall=0;
		$Dealer = M('Dealer');
		$map5=array();
		$map5['dl_belong']=$dlid;
		$map5['dl_unitcode']=session('unitcode');
		$count5 = $Dealer->where($map5)->count();
		if($count5>0){
			$countall=$countall+$count5;
			
			$map3=array();
			$map3['dl_belong']=$dlid;
			$map3['dl_unitcode']=session('unitcode');
			$list3 = $Dealer->where($map3)->order('dl_type ASC,dl_id DESC')->select();
			...
			foreach($list3 as $kk=>$vv){  
				...
				if($count4>0){
					$countall=$countall+$this->treesubcount($vv['dl_id']);
				}
			}
		}
		return $countall;
    ...
    public function searchtree(){
        ...
        if($dlusername!='' && $dlusername!='请填写代理账号'){
            ...
            $data2=$Dealer->where($map2)->find();
            if(!$data2){
				$this->error('请填写代理账号不存在','',1);
			}
            $dl_id=$data2['dl_id'];
			$dl_belong=$data2['dl_belong'];
        }
		if($dl_id<=0){
			$this->error('填写代理账号不正确','',1);
		}
		...
		$this->belong_arrs[]=array('id'=>$data2['dl_id'],'name'=>$data2['dl_name'],'username'=>$data2['dl_username'],'dltype'=>$data2['dl_type'],'status'=>$data2['dl_status']);

		//有效上家列表
		if($dl_belong>0){
			$map2=array();
			$map2['dl_id'] =  $dl_belong;
			$map2['dl_unitcode']=session('unitcode');
			$data2 = $Dealer->where($map2)->find();
			if($data2){
				$this->belong_arrs[]=array('id'=>$data2['dl_id'],'name'=>$data2['dl_name'],'username'=>$data2['dl_username'],'dltype'=>$data2['dl_type'],'status'=>$data2['dl_status']);
				if($data2['dl_belong']>0){
				   $this->get_alldlbelongarr($data2['dl_belong']);
				}
			}
		}
		$this->belong_arrs[]=array('id'=>0,'name'=>'总公司','username'=>'','status'=>'');
        
			
        
		$listhtml_1='';
		foreach($this->belong_arrs as $k=>$v){
			$listhtml_2='';
			if($v['id']==0){
				$listhtml_2.='<li>';
                $listhtml_2.='<img  src="'.__ROOT__.'/Public/mp/static/menu_minus.gif"  /> 总公司';
				$listhtml_2.='<ul id="s'.$v['id'].'" class="treeitem"  style="display:block">';
				if($listhtml_1==''){
					$listhtml_2.=$this->list_html;
				}else{
					$listhtml_2.=$listhtml_1;
				}
				...
				if($v['status']==0){
					$dl_status_str='[<span style="color:#009900" >新</span>]';
				}else if($v['status']==1){
					$dl_status_str='[正常]';
				}else if($v['status']==2){
					$dl_status_str='[代理已审]';
				}else if($v['status']==9){
					$dl_status_str='[<span style="color:#FF0000" >禁用</span>]';
				}else{
					$dl_status_str='[未知]';
				}
				
				$map2=array();
				$map2['dlt_id']=$v['dltype'];
				$map2['dlt_unitcode']=session('unitcode');
				$data2 = $Dltype->where($map2)->find();
				if($data2){
					$dl_type_str=' ('.$data2['dlt_name'].')';
				}else{
					$dl_type_str='';
				}
				...
				if($k==0){
					$listhtml_2.='<li >';
					$listhtml_2.='<img  src="'.__ROOT__.'/Public/mp/static/menu_minus.gif"  /> <a href="'.U('Mp/Dealer/treedetail?dl_id='.$v['id']).'"  target="dealerframe"  style="color:#FF0000" >'.htmlspecialchars($v['name']).'('.htmlspecialchars($v['username']).')</a>'.htmlspecialchars($dl_type_str).$dl_status_str.'';
					$listhtml_2.='<ul id="s'.$v['id'].'" class="treeitem" style="display:block">';
					$listhtml_2.=$this->list_html;
					$listhtml_2.='</ul>';
					$listhtml_2.='</li>';
					...
					$listhtml_2.='<li>';
					$listhtml_2.='<img  src="'.__ROOT__.'/Public/mp/static/menu_minus.gif"   /> <a href="'.U('Mp/Dealer/treedetail?dl_id='.$v['id']).'"  target="dealerframe"  >'.htmlspecialchars($v['name']).'('.htmlspecialchars($v['username']).')</a>'.htmlspecialchars($dl_type_str).$dl_status_str.'';
                    $listhtml_2.='<ul id="s'.$v['id'].'" class="treeitem" style="display:block" >';
                    $listhtml_2.=$listhtml_1;
					$listhtml_2.='</ul>';
					$listhtml_2.='</li>';
			...
			$listhtml_1=$listhtml_2;

		}
		

        $this->assign('list_html', $listhtml_1);

        $this->assign('curr', 'dealer_list');
        $this->display('searchtree');	
	}
	...
	public function review(){
        ...
        if($data){
           ...
           //当前状态
			...
			if($dltypeinfo){
				$data['dl_type_str']=$dltypeinfo['dlt_name'];
				$data['dl_type1']=$dltypeinfo['dlt_id'];
			}else{
				$data['dl_type_str']='-';
			}
            //经销商区域设置
//            判断等级
            ...
            $map3=array();
            $map3['dlt_unitcode']=session('unitcode');
            $map3['dl_id'] = $data['dl_id'];
            $dltypeinfo3 =  $Dealer->where($map3)->find();
            if($dltypeinfo3){
                $data['dl_sheng_str']=$dltypeinfo3['dl_sz_sheng'];
                $data['dl_shi_str']=$dltypeinfo3['dl_sz_shi'];
                $data['dl_qu_str']=$dltypeinfo3['dl_sz_qu'];
            }else{
                $data['dl_sheng_str']='-';
                $data['dl_shi_str']='-';
                $data['dl_qu_str']='-';
    ...
    public function active(){
        ...
        if($data){
            $active=intval(I('post.dl_status',0));
                    ...
                    //推荐返利 begin
						...
						if($data['dl_referee']>0 && $data3['dlt_fanli1']>0){
							...
							if($data4){
								//返利
								$Fanlidetail = M('Fanlidetail');
								//跨级推荐有推荐返利一次性，同级推荐有订单返利
								if($data4['dl_level']==2 && $data['dl_level']==1 ){
									$data5=array();
									$data5['fl_unitcode'] = session('unitcode');
									$data5['fl_dlid'] = $data4['dl_id']; //获得返利的代理
									$data5['fl_senddlid'] = $data['dl_belong']; //发放返利的代理 0为公司发放
									...
									$data5['fl_remark'] ='邀请 '.$data['dl_name'].'('.$data['dl_username'].') 成为 '.$data3['dlt_name'];
			...
			$odlog_arr=array(
						...
						'dlg_action'=>'审核/禁用经销商-'.$data['dl_status'].'-'.$active,
    ...
    public function update_dltype(){
        ...
        if($data){
            ...
            if($dltypeid>0){
				...
				//如果申请调级代理30天内申请到总代 推荐人是（合伙人）的有一次性返利
				/*
				if($apply_level==1){
					if((time()-$data['dl_startdate'])<3600*24*30  && ($data['dl_startdate']-$data['dl_addtime'])<3600*24*15){
						
						$map5 = array();
						$map5['od_oddlid'] = $data['dl_id'];
						...
						if($odcount>0){
							...
							$data4=$Dealer->where($map4)->find();
							if($data4){
								if($data4['dl_level']==2  && $dltinfo['dlt_fanli1']>0){
									$data5=array();
									$data5['fl_unitcode'] = session('unitcode');
									$data5['fl_dlid'] = $data4['dl_id']; //获得返利的代理
									$data5['fl_senddlid'] = 0; //发放返利的代理 0为公司发放
									$data5['fl_type'] = 1; // 返利分类 1-推荐返利 2-订单返利  11-提现减少返利 (1-10 增加返利 11-20 减少返利) 
									$data5['fl_money'] = $dltinfo['dlt_fanli1'];
									$data5['fl_refedlid'] = $data['dl_id']; //推荐返利中被推荐的代理
									$data5['fl_oddlid'] = 0; //订单返利中 下单的代理
									$data5['fl_odid'] = 0;  //订单返利中 订单流水id
									$data5['fl_orderid']  = ''; //订单返利中 订单id
									...
									$data5['fl_level']  = 1;  //返利的层次，1-第一层返利 2-第二层返利
									$data5['fl_addtime']  = time();
									$data5['fl_remark'] ='邀请 '.$data['dl_name'].'('.$data['dl_username'].') 30天内升级成为 '.$dltinfo['dlt_name'];
				...
				*/
				...
				$odlog_arr=array(
							...
							'dlg_dlusername'=>session('qyuser'),
							'dlg_dlname'=>session('qyuser'),
							'dlg_action'=>'修改经销商级别-'.$original_level.'-'.$apply_level,
							'dlg_type'=>0, //0-企业 1-经销商
							'dlg_addtime'=>time(),
							'dlg_ip'=>real_ip(),
							'dlg_link'=>__SELF__
				...
				foreach($dealerlist as $k=>$v){
					if($v['dl_belong']>0){
						...
						$odlog_arr=array(
									...
									'dlg_dlusername'=>session('qyuser'),
									'dlg_dlname'=>session('qyuser'),
									'dlg_action'=>'因推荐人调级更换上家-'.$v['dl_belong'].'-'.$map['dl_id'],
									'dlg_type'=>0, //0-企业 1-经销商
									'dlg_addtime'=>time(),
									'dlg_ip'=>real_ip(),
									'dlg_link'=>__SELF__
				...
				$map2=array();
				$map2['dl_unitcode']=session('unitcode');
				$map2['dl_belong']=$map['dl_id'];
				$map2['dl_level']=array('elt',$apply_level);
				$dealerlist = $Dealer->where($map2)->order('dl_id DESC')->select();
				foreach($dealerlist as $k=>$v){
    ...
    public function update_date(){
        ...
        if($data){
			...
			if($addyear>0){
				...
				$data2['dl_enddate'] =$begintime+3600*24*365*$addyear;
    ...
    public function update_belong(){
        ...
        if($data){
			...
			$odlog_arr=array(
						...
						'dlg_dlusername'=>session('qyuser'),
						'dlg_dlname'=>session('qyuser'),
						'dlg_action'=>'修改经销商上家-'.$data['dl_belong'].'-'.$dl_username,
						'dlg_type'=>0, //0-企业 1-经销商
						'dlg_addtime'=>time(),
						'dlg_ip'=>real_ip(),
						'dlg_link'=>__SELF__
    ...
    public function update_referee(){
        ...
        if($data){
			...
			if($data['dl_belong']==0){
				if($dl_referee==''){
					...
					if($data2){
						...
						//检测新推荐人是否在以前的推荐线上 以免进入死循环
						$myteam=$this->teamlist($map['dl_id']); 
						foreach ($myteam as $kk => $vv) {
							if($vv==$data2['dl_id']){
								$this->error('设置的推荐人已经在他的推荐线上');
							}
						}
			...
			$odlog_arr=array(
						...
						'dlg_dlusername'=>session('qyuser'),
						'dlg_dlname'=>session('qyuser'),
						'dlg_action'=>'修改经销商推荐人-'.$data['dl_referee'].'-'.$dl_referee,
						'dlg_type'=>0, //0-企业 1-经销商
						'dlg_addtime'=>time(),
						'dlg_ip'=>real_ip(),
						'dlg_link'=>__SELF__
				...
				if($apply_state==1){
					...
					if($data2){
						...
						if($dltypeid>0){
							...
							/*
							if($apply_level==1){
								if((time()-$data2['dl_startdate'])<3600*24*30  && ($data2['dl_startdate']-$data2['dl_addtime'])<3600*24*15){
									...
									if($odcount>0){
										...
										if($data4){
											if($data4['dl_level']==2  && $dltinfo['dlt_fanli1']>0){
												...
												$data5['fl_type'] = 1; // 返利分类 1-推荐返利 2-订单返利  11-提现减少返利 (1-10 增加返利 11-20 减少返利) 
												$data5['fl_money'] = $dltinfo['dlt_fanli1'];
												$data5['fl_refedlid'] = $data2['dl_id']; //推荐返利中被推荐的代理
												$data5['fl_oddlid'] = 0; //订单返利中 下单的代理
												$data5['fl_odid'] = 0;  //订单返利中 订单流水id
												$data5['fl_orderid']  = ''; //订单返利中 订单id
												$data5['fl_proid']  = 0;  //订单返利中 产品id
												...
												$data5['fl_level']  = 1;  //返利的层次，1-第一层返利 2-第二层返利
												$data5['fl_addtime']  = time();
												$data5['fl_remark'] ='邀请 '.$data2['dl_name'].'('.$data2['dl_username'].') 30天内升级成为 '.$dltinfo['dlt_name'];
							...
							foreach($dealerlist as $k=>$v){
								if($v['dl_belong']>0){
									...
									$odlog_arr=array(
												...
												'dlg_dlusername'=>session('qyuser'),
												'dlg_dlname'=>session('qyuser'),
												'dlg_action'=>'因推荐人调级更换上家-'.$data2['dl_id'],
												'dlg_type'=>0, //0-企业 1-经销商
												'dlg_addtime'=>time(),
												'dlg_ip'=>real_ip(),
												'dlg_link'=>__SELF__
