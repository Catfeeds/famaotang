...
class JfmobiController extends CommController {
    ...
    public function basic(){
        ...
        if($action=='profile'){
            ...
            $this->assign('curr', 'jfmo_wenda');			
            $this->display('help');       //帮助中心
        }elseif($action=='buyer'){      
            $this->assign('curr', 'jfmo_xuzhi');			
            $this->display('buyer');       //代理须知
			
        }elseif($action=='zhengce'){     
            $this->assign('curr', 'jfmo_zhengce');		
    ...
    public function newslist(){
        ...
        foreach($list as $k=>$v){ 
            ...
            if($v['news_type']==1){
				...
				$list[$k]['news_type_str']='传播素材';
            }else if($v['news_type']==5){
                $list[$k]['news_type_str']='招商政策';
            }else if($v['news_type']==7){
                $list[$k]['news_type_str']='商家活动';
			}else if($v['news_type']==9){
                $list[$k]['news_type_str']='知识库';
			}else if($v['news_type']==10){
                $list[$k]['news_type_str']='代理须知';
            }else{
