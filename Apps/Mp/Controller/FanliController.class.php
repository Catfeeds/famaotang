...
class FanliController extends CommController {
	...
	public function make_salemonthly(){
		...
		if($action=='save'){
			...
			if($upyearmonth<strtotime($nyearmonth) && $upyearmonth!==FALSE){
				...
				if((time()-$endtime)>3600*6*15){ //当月15号后才允许生成

				}else{
					$this->error('当月15日后生成上月的奖励','',2);
					exit;
				}
				...
				foreach ($list as $k => $v) {
					...
					//有奖金则记录奖金
					if($reward>0){
                        $shouxufei=$reward*C('FANLI_SHOUXUFEI');
						$data7=array();
						...
						$data7['fl_shouxufei'] = sprintf("%.2f", $shouxufei);
    ...
    public function teamlist($id,$dllevel=0){
        $this->check_qypurview('14005',1);
