...
//			dunp($data2);die();
			...
			//允许操作
			$caozuostr='<a href="'.U('./Mp/Orders/orderdetail?od_id='.$v['od_id'].'&odbl_id='.$v['od_id'].'').'"  >订单详细</a>　';
		...
		if($od_id>0){
			...
			if($data){
				...
				if($data['od_state']==1 || $data['od_state']==2 || $data['od_state']==3 ){
					if($data['od_express']>0){
						$caozuostr='<div class="ui-btn ui-btn-primary"   data-href="'.U('./Mp/Orders/odfinishship?od_id='.$data['od_id'].'&od_state='.$od_state.'').'"  >修改物流</div> ';
					}else{
						$caozuostr='<div class="ui-btn ui-btn-primary"   data-href="'.U('./Mp/Orders/odfinishship?od_id='.$data['od_id'].'&od_state='.$od_state.'').'"  >完成发货</div> ';
					}
				}
				
				//取消订单
				//if($data['odbl_state']==0 || $data['odbl_state']==1 ){
				//	$caozuostr.='<div class="ui-btn ui-btn-primary"   data-href="'.U('./Mp/Orders/canceldlorder?state=9&od_id='.$data['od_id'].'&od_state='.$od_state.'').'"  >取消订单</div> ';
				//}
				...
				$flarr = $Fanlidetail->where($map2)->order('fl_addtime DESC')->limit(100)->select();
				foreach($flarr as $kkk=>$vvv){
					//收款代理信息
					if($vvv['fl_dlid']>0){
						$map3=array();
						$map3['dl_id']=$vvv['fl_dlid'];
						$map3['dl_unitcode']=session('unitcode');
						$data3=$Dealer->where($map3)->find();
						if($data3){
							$flarr[$kkk]['dl_name_str']=$data3['dl_name'].'('.$data3['dl_username'].')';
						}else{
							$flarr[$kkk]['dl_name_str']='-';
						}
					}else{
						$flarr[$kkk]['dl_name_str']='-';
					}
                    //付款代理信息
					if($vvv['fl_senddlid']>0){
						$map3=array();
						$map3['dl_id']=$vvv['fl_senddlid'];
						$map3['dl_unitcode']=session('unitcode');
						$data3=$Dealer->where($map3)->find();
						if($data3){
							$flarr[$kkk]['dl_sendname_str']=$data3['dl_name'].'('.$data3['dl_username'].')';
						}else{
							$flarr[$kkk]['dl_sendname_str']='-';
						}
					}else{
					...
					if($vvv['fl_type']>=1 && $vvv['fl_type']<=10){
						...
						$flarr[$kkk]['fl_moneystr']='<span style="color:#009900">-'.number_format($vvv['fl_money'], 2,'.','').'</span>';
					}else{
						$flarr[$kkk]['fl_moneystr']=number_format($vvv['fl_money'], 2,'.','');
					}
					
					if($vvv['fl_type']==1){
						$flarr[$kkk]['fl_typestr']='推荐返利';
					}else if($vvv['fl_type']==2){
						$flarr[$kkk]['fl_typestr']='订单返利';
					}else if($vvv['fl_type']==3){
						$flarr[$kkk]['fl_typestr']='销售累计奖';
					}else if($vvv['fl_type']==4){
						$flarr[$kkk]['fl_typestr']='按月销售奖';
					}else if($vvv['fl_type']==5){
					...
					if($vvv['fl_state']==0){
						$flarr[$kkk]['fl_state_str']='待收款';
					}else if($vvv['fl_state']==1){
						$flarr[$kkk]['fl_state_str']='已收款';
					}else if($vvv['fl_state']==2){
						$flarr[$kkk]['fl_state_str']='收款中';
					}else if($vvv['fl_state']==9){
						$flarr[$kkk]['fl_state_str']='已取消';
					}else{
						$flarr[$kkk]['fl_state_str']='-';
					}
				}
	...
	public function odshiplist(){
		...
		if($od_id>0 && $oddt_id>0){
			...
			$map['ship_deliver']=$order['od_rcdlid'];   //ship_deliver--出货方   ship_dealer--收货方
			$map['ship_odid']=$od_id;
...
//            }
    ...
    public function cporderdetail(){
		...
		// $odbl_id=intval(I('get.odbl_id',0));
		$back=intval(I('get.back',0));
//        $od_fhdlid=intval(I('get.od_fhdlid',0));//发货人
//        dump($od_fhdlid);die;
		if($od_id>0){
			$Orders=M('Orders');
			$map=array();
...
//		发货人
        $map20['dl_id']=$data['od_fhdlid'];
        $_data20=$Dealer->where($map20)->find();
        if($_data20){
            $data20=$_data20['dl_name'];
            $data21=$_data20['dl_username'];
        }else{
            $data20='';
            $data21='总公司';
        }
//		dump($_data20);die();
    ...
    public function xnorders(){
        ...
        foreach($list as $k=>$v){
			...
			//允许操作
			$caozuostr='<a href="'.U('./Mp/Orders/xnorderdetail?od_id='.$v['od_id'].$v['od_id'].'').'"  >订单详细</a><br>';
					...
					if($data2){
						...
						if($orderdealer['dl_referee']>0){
							...
							if($data4){
								...
								if($data['od_rcdlid']!=$data4['dl_id'] && $order_level==$data4['dl_level']){
									//返利给推荐人
			 ...
			 if($data['od_express']<=0){
				...
				$fanli_dllevel3=0;
			 	...
			 	if($orderdealer){
					...
					////=========================================推荐返利 一次性返利
					if($orderdealer['dl_level']==1 || $orderdealer['dl_level']==2 || $orderdealer['dl_level']==3 || $orderdealer['dl_level']==4|| $orderdealer['dl_level']==5){
						
						//是不是首次下单
						$map51 = array();
						$map51['od_oddlid'] = $orderdealer['dl_id'];
						$map51['od_unitcode'] = session('unitcode');
						$map51['od_state'] = 8;
						...
						if($data4){
							...
							$map2=array();
							$map2['tfl_unitcode']=session('unitcode');
...
//                                        $data5['fl_money'] = $data['od_total']*C('LOW_HIGH');
//                                        $shouxufei=$data['od_total']*C('LOW_HIGH')*0.1;
...
//                                }
					...
					//=========================================订单返利
			 		...
			 		$map2=array();
			 		$map2['pfl_unitcode'] = session('unitcode');
			 		$map2['pfl_tjdltype'] = $orderdealer['dl_type'];//$orderdealer为下单代理
			 		if($proids){
			 			$map2['pfl_proid'] = array('IN',$proids);
			 		}
			 		$data2=$Profanli->where($map2)->find(); //是否有设置返利
                                                ...
                                                if (!$data8) {
                                                    ...
                                                    */
                                                    ...
                                                    if ($orderdealer['dl_level'] == 1) {
                                                        ...
                                                        $map30['dl_unitcode'] = session('unitcode');
...
//                                                                      正常返利
                                                                    $pfl_fanli1sum = $pfl_fanli1 * $vv['oddt_qty']*C('B_MONEY');
                                                    ...
                                                    if ($orderdealer['dl_level'] == 1) {
                                                        ...
                                                        if ($order_dltype) {
                                                            if ($order_dltype['dlt_minnum'] != 0) {
                                                                ...
                                                                if ($data['od_total'] <= $dlt_minnum) {
//                                                                        正常返利
                                                                    $pfl_fanli2sum = $pfl_fanli2 * $vv['oddt_qty']*C('A_MONEY');
...
//                                    补货人的返利
                                    foreach($oddetail as $kk=>$vv) {
                                        ...
                                        $pfl_fanli3sum = $pfl_fanli3 * $vv['oddt_qty']*C('C_MONEY');
                                        $shouxufei = $pfl_fanli3sum * C('FANLI_SHOUXUFEI');

                                        $data5 = array();
                                        $data5['fl_unitcode'] = session('unitcode');
                                        $data5['fl_dlid'] = $orderdealer['dl_id']; //获得返利的代理
                                        $data5['fl_senddlid'] = 0; //发放返利的代理 0为公司发放
                                        $data5['fl_type'] = 2; //返利分类 1-推荐返利 2-订单返利  11-提现减少返利 (1-10 增加返利 11-20 减少返利)
...
//
               //--------------------------升级------------------------------
                 ...
                 $dlt_id4=$dlt_type4['dlt_id'];
//                 1、累计补货升级
                 $upgrade=[];
                 $upgrade=$Orders->where(['od_state'=>3,'od_oddlid'=>$orderdealer['dl_id']])->select();
                 foreach ($upgrade as $k=>$v){
                     $upgrade1= $Orderdetail->where(['oddt_odid'=>$v['od_id']])->select();
                     $upgrade[$k]['upgrade1']=$upgrade1;
                 }
                 $upgrade_num=0;
                 foreach($upgrade as $k=>$v){
                     foreach ($v['upgrade1'] as $k1=>$v1){
                         $upgrade_num+=$v1['oddt_qty'];
                     }
                 }
//                 代理商
                 if($orderdealer['dl_level']==5){
...
//                 2、一次性进货升级
                 ...
                 $upgrade_num2=0;
                 foreach ($upgrade2 as $k=>$v){
                     $upgrade_num2+=$v['oddt_qty'];
                 }
//                 代理商
                 if($orderdealer['dl_level']==5){
                     if($upgrade_num2>=36){
