...
class DealerController extends CommController {
    ...
    public function index(){
		...
		$map['dlt_level']=array('IN','1,2,3,4');
		...
		$map['dlt_unitcode']=$this->qy_unitcode;
        $map['dlt_level']=array('IN','1,2,3,4');
		$data3 = $Dltype->where($map)->field('dlt_id,dlt_name,dlt_level')->order('dlt_level ASC,dlt_id ASC')->select();
		foreach($data3 as $kk=>$vv){
			$map2=array();
			$count2=0;
			$map2['dl_unitcode']=$this->qy_unitcode;
			$map2['dl_referee']=session('jxuser_id');
			$map2['dl_type']=$vv['dlt_id'];
			$map2['dl_status']=array('in','0,1,2');
			$count2 = $Dealer->where($map2)->count();
			$data3[$kk]['count']=$count2;
		}
		...
		$map3['dl_status']=0;
		$dl_list= $Dealer->where($map3)->field('dl_id,dl_name,dl_weixin,dl_wxheadimg,dl_tel,dl_address,dl_addtime,dl_level,dl_type')->order('dl_id DESC')->select();
		foreach($dl_list as $kk=>$vv){
			$map=array();
			$map['dlt_unitcode']=$this->qy_unitcode;
			$map['dlt_id']=$vv['dl_type'];
	...
	public function login(){
		...
		if($action=='login'){
			...
			if($data){
				if($data['dl_pwd']==$md5_pwd){
					if($data['dl_status']==1){
						...
						cookie('jxuser_check',$jxuser_check,360000);
			...
			$nowtime=time();
			if(MD5($fwkey.$ttamp.$qy_fwsecret)!=$sture){
				$this->error('请求超时，请重试',U(C('MODULE_NAME').'/Dealer/index'),1);
			}
			if(($nowtime - $ttamp) > 1200) {
				$this->error('请求超时，请重试',U(C('MODULE_NAME').'/Dealer/index'),1);
			}
	...
	public function authorization(){
		...
		if($data){
			...
			$shouquandate1=date("Y　　　m　　d", $data['dl_startdate']);
			$shouquandate2=date("Y　　m　　d",$data['dl_enddate']);

			$imgpath = BASE_PATH.'/public/uploads/dealer/';
			
			if($data['dl_pic']!='' && file_exists($imgpath.$data['dl_pic']) && (time()-filemtime($imgpath.$data['dl_pic']))<300 ){
				...
				$ttfpath = BASE_PATH.'/public/'.C('RESOURCE_ROOT_NAME').'/static/font.ttf'; 
				$temppic = BASE_PATH.'/public/'.C('RESOURCE_ROOT_NAME').'/static/shouquantemp.jpg';
				...
				$pictext=array();
				$pictext[]=array('txt'=>$data['dl_name'],'x'=>282,'y'=>395,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
				$pictext[]=array('txt'=>$dl_weixin,'x'=>282,'y'=>469,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
				$pictext[]=array('txt'=>$dl_tel,'x'=>282,'y'=>514,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
				//$pictext[]=array('txt'=>$dl_idcard,'x'=>420,'y'=>196,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
				$pictext[]=array('txt'=>$dlt_namestr,'x'=>282,'y'=>433,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
				$pictext[]=array('txt'=>$shouquandate1,'x'=>177,'y'=>779,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
				$pictext[]=array('txt'=>$shouquandate2,'x'=>399,'y'=>779,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
				$pictext[]=array('txt'=>'广州发茂堂生物科技有限公司','x'=>390,'y'=>856,'color'=>array('34','34','34'),'font'=>$ttfpath,'fontsize'=>'12');
	   ...
	   if(!$this->is_jxuser_login()){
		...
		if($data){
			...
			if($data['dl_stockpic']!='' && file_exists($imgpath.$data['dl_stockpic']) && (time()-filemtime($imgpath.$data['dl_stockpic']))<1800 ){
				...
				//生成授权书 只支持jpg 根据每个授权证书的图片不同要调整文字坐标位置
				$ttfpath = BASE_PATH.'/public/'.C('RESOURCE_ROOT_NAME').'/static/font.ttf'; 
				$temppic = BASE_PATH.'/public/'.C('RESOURCE_ROOT_NAME').'/static/stockcertificate.jpg';
				$picfilename = $this->qy_unitcode.'/'.time().'_'.$data['dl_id'].'_'.mt_rand(1000,9999).'.jpg';
				$savepic = BASE_PATH.'/public/uploads/dealer/'.$picfilename;
				if(!is_dir(BASE_PATH.'/public/uploads/dealer/'.$this->qy_unitcode)){
					@mkdir(BASE_PATH.'/public/uploads/dealer/'.$this->qy_unitcode);
	...
	public function invite(){
		...
		$data2 = $sharelinks->where($map2)->order('sl_id DESC')->limit(1)->select();
		$incount=count($data2);
		$Dltype = M('Dltype');
		foreach($data2 as $k=>$v){ 
			...
			*/
			...
			if(($v['sl_endtime']-time())>0){
				...
				//删除已过期
				$map3=array();
				$map3['sl_unitcode']=$this->qy_unitcode;
				$map3['sl_dealerid']=session('jxuser_id');
				$map3['sl_id']=$v['sl_id'];
				$sharelinks->where($map3)->delete(); 
				$filepath = BASE_PATH.'/Public/uploads/dealer/'.$this->qy_unitcode.'/';
		        $filename = 'shareqr_'.$this->qy_unitcode.'_'.$v['sl_id'].'.png';
				$filename2 = 'shareqr_'.$this->qy_unitcode.'_'.$v['sl_id'].'.jpg';
				
				@unlink($filepath.$filename);
				@unlink($filepath.$filename2);
				unset($data2[$k]);
			}
		}
	...
	public function marklinks(){
		...
		if($slid>0){
			...
			//if($dltypeid>0){		
				...
				*/
			//}else{
				//$this->error('请选择邀请级别',U(C('MODULE_NAME').'/Dealer/invite'),2);
			//}
			header('location:'.WWW_WEBROOT.C('MODULE_NAME').'/Dealer/marklinks/slid/'.$slid.'');
			exit;
		}
		...
		//生成二维码
		$http_host=WWW_WEBHOST.WWW_WEBROOT;
		$shlink=$http_host.C('MODULE_NAME').'/dealer/apply/slid/'.$slid.'/jxid/'.session('jxuser_id');

		
		$filepath = BASE_PATH.'/Public/uploads/dealer/'.$this->qy_unitcode.'/';
		$filename = 'shareqr_'.$this->qy_unitcode.'_'.$slid.'.png';
		...
		if(file_exists($filepath.$filename)){
			...
			make_ercode($shlink,$filepath.$filename,'','','Q',3);
		}
		$temppic=BASE_PATH.'/Public/'.C('RESOURCE_ROOT_NAME').'/static/sharetemp1.jpg';
		$filename2 = 'shareqr_'.$this->qy_unitcode.'_'.$slid.'.jpg';
		$savepic = $filepath.$filename2;
		$qrpic=$filepath.$filename;
		if(file_exists($filepath.$filename2)){
		}else{
		    make_shareqrpic($temppic,$savepic,$qrpic,253,50);
		}
		
		$shareqr_str='<img src="'.__ROOT__.'/Public/uploads/dealer/'.$this->qy_unitcode.'/'.$filename2.'"   border="0" style="vertical-align:middle;width:90%" >';
        ...
        $this->assign('shareqr_str', $shareqr_str);
		$this->assign('shtitle', C('SHARE_TITLE'));
		$this->assign('shdesc', C('SHARE_DESC'));
		$this->assign('shlink', WWW_WEBHOST.WWW_WEBROOT.C('RESOURCE_ROOT_NAME').'/dealer/apply/slid/'.$slid.'/jxid/'.session('jxuser_id'));
		$this->assign('shimgurl', WWW_WEBHOST.WWW_WEBROOT.'public/'.C('RESOURCE_ROOT_NAME').'/static/fenxiang.jpg');
		$this->assign('ba_picstr', $ba_picstr);
		$this->assign('timing', $timing);
		$this->assign('dlt_name', $dlt_name);
		
		$this->assign('http_host', WWW_WEBHOST.WWW_WEBROOT);
	...
	public function apply(){
	    ...
	    if ($action=='save') {
			...
			$nowtime=time();
			if(MD5($fwkey.$ttamp.$qy_fwsecret)!=$sture){
				$this->error('请求超时，请重试',U(C('MODULE_NAME').'/Dealer/apply?slid='.$slid.'&jxid='.$jxid.''),2);
			}
			if(($nowtime - $ttamp) > 1200) {
				$this->error('请求超时，请重试',U(C('MODULE_NAME').'/Dealer/apply?slid='.$slid.'&jxid='.$jxid.''),2);
			}
			...
			if(!preg_match("/^[0-9]{6,6}$/",$checkcode)){
				$this->error('请填写验证码','',2);
			}
			*/
            ...
            */
			...
			if($dlreferee){
				...
				if($data3){
					...
					$this->error('该邀请链接已失效',U(C('MODULE_NAME').'/Index/index'),2);
				}
			}else{
...
//			筛选：1、（大城市：区是否只有一个旗舰店，如果有，那该区的订单都归该旗舰店发货，则该区允许有多个体验店；如果没有，判断是否有体验店，如果有，那该区的订单都归该体验店发货，那该区只允许有一个；否则由公司发货）
//			dump($dl_prov);
//			dump($dl_city);
//			dump($dl_area);die();
//            $dltid
...
//            $map4['dl_sheng']=$dl_prov;
//            $map4['dl_shi']=$dl_city;
//            $map4['dl_qu']=$dl_area;
//            $map4['dl_type']=$dltid;
//            $sheng_city_qu=$Dealer->where($map4)->find();
//            if($sheng_city_qu){
//                $this->error('对不起，该区','',2);
//            }
			...
			//验证码验证
			/*
			$Smsvalidate= M('Smsvalidate');
			$map2=array();
			$map2['sms_unitcode']=$this->qy_unitcode;
			$map2['sms_tel']=$dl_tel;
			$map2['sms_code']=$checkcode;
			$map2['sms_sturestr']=$sture;
			$map2['sms_addtime']=array('EGT',time()-600);
			$checkcodeinfo=$Smsvalidate->where($map2)->find();
			if($checkcodeinfo){
			}else{
				$this->error('验证码不正确或已过期','',2);
			}
			*/
			...
			if($apply_level==1 || $apply_level==2 || $apply_level==3 ||  $apply_level==4){  //前4级上家为公司
			    $dlbelong_id=0;  //上家id
			    $dlbelong_name='总公司'; //上家名称
			}else{
				if($apply_level<=$dlreferee_level){  //如果申请的级别高于推荐人的级别 或 与推荐人同级
					if($dlreferee_belong>0){
						...
						if($dlbelong_id===false){
							$this->error('该邀请链接已失效 8',U(C('MODULE_NAME').'/Index/index'),2);
						}else{
							if($dlbelong_id>0){
								$map=array();
								...
								if($dlbelong){
									...
									$this->error('该邀请链接已失效 9',U(C('MODULE_NAME').'/Index/index'),2);
								}
							}else{
								$dlbelong_id=0;
							}
					...
					if($dlbelong){
						...
						$this->error('该邀请链接已失效 10',U(C('MODULE_NAME').'/Index/index'),2);
					}
				}
			}
			...
			if($result){
				...
				$map['dl_unitcode']=$this->qy_unitcode;
				$data=array();
				$data['dl_number'] ='FMT'.str_pad($result,7,'0',STR_PAD_LEFT);
				$Dealer->where($map)->save($data);
				

				
				$this->success('您的申请提交成功，请等待审核，用户名：'.$dl_weixin.' 密码：'.substr($dl_tel,-6).'',U(C('MODULE_NAME').'/Index/index'),999);
				exit;
			}else{
				$this->error('申请提交失败,请重新填写提交',U(C('MODULE_NAME').'/Index/index'),2);
			}
		}
	    /////////////////////////////////////////////////////////////
        //===========================申请表单
        ...
        if($slid>0 && $jxid>0){
			...
			*/
			...
			$dlbelong_name='总公司';
            /*
			if($apply_level==1 || $apply_level==2){
				//前2级上家为公司
			}else{
				if($apply_level<=$dlreferee_level){  //如果申请的级别高于推荐人的级别 或 与推荐人同级
					if($dlreferee_belong>0){
    ...
    public function dealerlist(){
		...
		if($dltid>0){
			...
			if($dltinfo){
				...
				$this->error('没有该记录',U(C('MODULE_NAME').'/Dealer/brandlevel'),'',2);
			}
			...
			$this->error('没有该记录',U(C('MODULE_NAME').'/Dealer/index'),'',2);
		}
	}
	...
	public function dealerdetail(){
		...
		$map['dl_unitcode']=$this->qy_unitcode;
		$where=array();
		$where['dl_belong']=session('jxuser_id');
		$where['dl_referee']=session('jxuser_id');
		$where['_logic'] = 'or';
		$map['_complex'] = $where;
		...
		if($data){
			...
			$this->error('没有该记录',U(C('MODULE_NAME').'/Dealer/applylist'),1);
		}
    ...
    public function refereelist(){
		...
		if($dltid>0){
			...
			if($dltinfo){
				...
				$this->error('没有该记录',U(C('MODULE_NAME').'/Dealer/referee'),'',2);
			}
		...
		if($dlid>0){
			...
			if($data){
				...
				if($rs){
					...
					//如果有推荐人 并设置了返利
					/*
					if($data['dl_referee']>0 && $data3['dlt_fanli1']>0 && $data['dl_startdate']<=0 ){
						$map4=array();
						$data4=array();
						...
						if($data4){
							...
							//前2级跨级推荐1级有推荐返利
							if($refedlt_level==2 && $applydlt_level==1 ){					
								//返利
								...
								$data5['fl_remark'] ='邀请 '.$data['dl_name'].'('.$data['dl_username'].') 成为 '.$data3['dlt_name'].' 推荐返利';
					...
					*/
					...
					$this->success('审核成功',U(C('MODULE_NAME').'/Dealer/applylist'),1);
				}else{
					$this->error('审核失败',U(C('MODULE_NAME').'/Dealer/applylist'),1);
				}
			}else{
				$this->error('没有该记录',U(C('MODULE_NAME').'/Dealer/applylist'),1);
			}
	...
	public function applydelete(){
		...
		if($data){
            ...
            $this->success('删除成功',U(C('MODULE_NAME').'/Dealer/applylist'),1);
		}else{
			$this->error('没有该记录',U(C('MODULE_NAME').'/Dealer/applylist'),'',1);
		}
	}
    ...
    public function shdelete(){
        ...
        if($data){
			...
			$this->success('删除成功',U(C('MODULE_NAME').'/Dealer/shiplist'),2);
        }else{
            $this->error('没有该记录','',2);
        }     
    }
	...
	public function qyshare(){
		...
		if($dltid>0){
			...
			$shlink=WWW_WEBHOST.WWW_WEBROOT.C('RESOURCE_ROOT_NAME').'/dealer/qyjxapply/dltid/'.$dltid.'/ttamp/'.$ttamp2.'/sture/'.$sture2;
   ...
   //经销商出货扫描
    public function shipscan(){
		...
		if(is_not_null($jxoddcount)){
			...
			$this->redirect(C('MODULE_NAME').'/Dealer/makeshipodd','' , 0, ''); //创建出货单号
		}
    ...
    public function makeshipodd(){
		...
		if($action=='save'){
			...
			if($data){
				...
				$data2=$Dealer->where($map2)->find();
				if($data2){
					$this->redirect(C('MODULE_NAME').'/Dealer/makeshipodd','' , 0, '');
				}else{
					$data3=array();
					...
					if($rs){
						...
						$this->redirect(C('MODULE_NAME').'/Dealer/shipscan','' , 0, '');
					}else{
						$this->error('创建出货单号失败',U(C('MODULE_NAME').'/Dealer/index'),2);
					}
				}
			}else{
				$this->error('授权已过期，请重新登录',U(C('MODULE_NAME').'/Dealer/index'),1);
				exit;
			}
		}
    ...
    public function shipscanlist(){
		...
		if(count($brcodearr)<=0){
			$this->error('没有扫描纪录',U(C('MODULE_NAME').'/Dealer/shipscan'),2);
		}
   ...
   //经销商出货扫描记录-移除
    public function shipremove(){
        ...
        $this->success('移除成功',U(C('MODULE_NAME').'/Dealer/shipscanlist'),1);
	}
	...
	public function updatedltypedetail(){
		...
		if($data){
			...
			if(is_not_null($data['apply_pic']) && file_exists($imgpath.$data['apply_pic'])){
				$data['apply_pic_str']='<a href="'.__ROOT__.'/Public/uploads/dealer/'.$data['apply_pic'].'"  target="_blank" ><img src="'.__ROOT__.'/Public/uploads/dealer/'.$data['apply_pic'].'"   border="0" style="vertical-align:middle;width:30%"  ></a>';
			...
			$this->error('没有该记录',U('./Famaotang/Dealer/updatedltypeindex/up_status/0'),2);
		}
	...
	public function updatedltype(){
		...
		$map2=array();
        $map2['dlt_unitcode']=$this->qy_unitcode;
		$map2['dlt_level']=array(array('lt',$data['original_level']),array('egt',1),'and');
        $dltypelist = $Dltype->where($map2)->field('dlt_id,dlt_name,dlt_level')->order('dlt_level ASC,dlt_id ASC')->select();
		foreach($dltypelist as $kk=>$vv){
			 $dltypelist[$kk]['dlt_name']='　　'.$vv['dlt_name'].'　　';
		}
	...
	public function updatedltype_save(){
		...
		$nowtime=time();
		if(MD5($fwkey.$ttamp.$qy_fwsecret)!=$sture){
			$this->error('请求超时，请重试',U(C('MODULE_NAME').'/Dealer/updatedltype'),2);
		}
		if(($nowtime - $ttamp) > 1200) {
			$this->error('请求超时，请重试',U(C('MODULE_NAME').'/Dealer/updatedltype'),2);
		}
		...
		if($dltypeid>0){
			...
			}else
			{
				...
				if($rs3){
				    $rs4=$Applydltype->add();
				    if($rs4){
					   $this->success('申请提交成功，请等待审核处理',U(C('MODULE_NAME').'/Dealer/updatedltypeindex/up_status/1'),2);
					   exit;
				    }else{
					   $this->error('提交失败','',1);
					   exit;
