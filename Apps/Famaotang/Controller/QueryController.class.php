...
class QueryController extends CommController {    
    ...
    public function index(){
        ...
        if($fwcode!=''){
			...
			$referer=substr($referer,0,300).'_Famaotang';
			...
			$url=WWW_WEBHOST.WWW_WEBROOT.'fwapi/Getres?fwkey='.urlencode($this->qy_fwkey).'&ip='.urlencode($ip).'&fwcode='.urlencode($fwcode).'&signature='.urlencode($signature).'&timestamp='.urlencode($timestamp).'&referer='.urlencode($referer);  
			...
			if($data===FALSE){
				...
				if(!is_array($data_arr)){
					...
					$msg='您查询的防伪码：'.$fwcode.',查询结果：'.$data_arr['msg'];
    ...
    public function ajaxres(){
        if (IS_POST) {
			...
			$msg=array('fwc'=>'','stat'=>'5','msg'=>'请正确输入防伪码1');
       ...
       if($referer==''){
        ...
        if($fwcode==''){
			$msg=array('fwc'=>'','stat'=>'5','msg'=>'请正确输入防伪码2');
			echo json_encode($msg);
			exit;
        }
		if($fwcode!=''){
			if(!preg_match("/^[0-9]{10,27}$/",$fwcode)){
				$msg=array('fwc'=>'','stat'=>'5','msg'=>'请正确输入防伪码3');
				echo json_encode($msg);
				exit;
			}
		}
		...
		if($qydata){
            ...
            $msg=array('fwc'=>$fwcode,'stat'=>'5','msg'=>'请正确输入防伪码4');
            echo json_encode($msg);
		    exit;
		}
        ...
        $url=WWW_WEBHOST.WWW_WEBROOT.'fwapi/Getres?fwkey='.urlencode($fwkey).'&ip='.urlencode($ip).'&fwcode='.urlencode($fwcode).'&signature='.urlencode($signature).'&timestamp='.urlencode($timestamp).'&referer='.urlencode($referer);  
        
        $data=@file_get_contents($url);
        if($data===FALSE){
            $msg=array('fwc'=>$fwcode,'stat'=>'5','msg'=>'请正确输入防伪码5');
            echo json_encode($msg);
            exit;
        }else{
            $data_arr=json_decode($data,true); 
            if(!is_array($data_arr)){
                $msg=array('fwc'=>$fwcode,'stat'=>'5','msg'=>'请正确输入防伪码6');
                echo json_encode($msg);
                exit;
            }else{
