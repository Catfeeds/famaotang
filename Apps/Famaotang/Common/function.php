...
//生成推广图片 带二维码 模板图只支持jpg   二维码图片 png格式
function make_shareqrpic($temppic='',$savepic='',$qrpic='',$x=0,$y=0){
	if($temppic=='' || $savepic==''  || $qrpic==''){
		exit;
	}
	$source_handle = imagecreatefromjpeg($temppic); //模板
	$qrpic_handle = imagecreatefrompng($qrpic); //二维码
	
    $bj_width = imagesx($source_handle);  //背景宽
	$bj_height = imagesy($source_handle);  //背景高
	$img_width = imagesx($qrpic_handle);
	
	imagecopyresampled($source_handle, $qrpic_handle, $x, $y, 0, 0, $img_width, $img_width, $img_width, $img_width);

    imagejpeg($source_handle,$savepic);
    imagedestroy($source_handle);
	imagedestroy($qrpic_handle);
}
